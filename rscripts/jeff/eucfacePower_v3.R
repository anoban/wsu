...
# adjusted so that random effects can be constant (arguments 'const.ranef.ring', 'const.ranef.tree')

tryCatch.converg <- function(expr) {
    converged <- T

    w.handler <- function(w) {
        converged <<- F # <<- operator targets the parent scope, akin to global variable_name in python!
...
eucface.power <- function(file.out, effect.size, n.tree, n.leaf, const.ranef.ring = NULL, const.ranef.tree = NULL, ranef.ring.sd = NULL, ranef.tree.sd = NULL, rand.sd, intercept, n.ring = 6) {
    n.ring <- n.ring
    n.tree <- n.tree
    n.leaf <- n.leaf
    ...
    if (is.null(const.ranef.ring)) {
        ...
        ranef.ring <- rep(const.ranef.ring, each = n.leaf * n.tree)
    }
    if (is.null(const.ranef.tree)) {
        ...
        ranef.tree <- rep(const.ranef.tree, each = n.leaf)
    }
    ...
    if (n.tree > 1 & n.leaf > 1) {
        ...
        if (any(c(!is.null(ranef.ring.sd), !is.null(const.ranef.ring))) & all(c(is.null(ranef.tree.sd), is.null(const.ranef.tree)))) {
            mod.alt <- tryCatch.converg(lmer(response ~ trt + (1 | ring), REML = F, control = list(maxIter = 1000)))
            mod.nul <- tryCatch.converg(lmer(response ~ 1 + (1 | ring), REML = F, control = list(maxIter = 1000)))

            ring.sd <- attr(VarCorr(mod.alt$value)[["ring"]], "stddev")[["(Intercept)"]]
            tree.sd <- NA
        }
        if (all(c(is.null(ranef.ring.sd), is.null(const.ranef.ring))) & any(c(!is.null(ranef.tree.sd), !is.null(const.ranef.tree)))) {
            mod.alt <- tryCatch.converg(lmer(response ~ trt + (1 | tree), REML = F, control = list(maxIter = 1000)))
            mod.nul <- tryCatch.converg(lmer(response ~ 1 + (1 | tree), REML = F, control = list(maxIter = 1000)))
    ...
    if (!is.null(const.ranef.ring)) ranef.ring.sd <- T
    if (is.null(ranef.tree.sd)) ranef.tree.sd <- NA
    if (!is.null(const.ranef.tree)) ranef.tree.sd <- T
