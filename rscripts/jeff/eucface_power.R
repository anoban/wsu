...
extracts$month.chr <- as.character(extracts$month)
extracts$month.chr[extracts$month < 10] <- paste("0", extracts$month.chr[extracts$month < 10], sep = "")
extracts$year <- year(extracts$Date)
extracts$yearMonth <- factor(with(extracts, paste(year, month.chr, sep = "-")))

extracts <- extracts[extracts$Date <= "2012-09-03", ]
extracts <- droplevels(extracts)
...
iems$month.chr <- as.character(iems$month)
iems$month.chr <- paste("0", iems$month.chr, sep = "")
iems$year <- year(iems$Date)
iems$yearMonth <- factor(with(iems, paste(year, month.chr, sep = "-")))

iems <- iems[!is.na(iems$Date), ]
iems <- droplevels(iems)
...
AICc(mod1.tree)
AICc(mod1.ring)
AICc(lm(percentN ~ 1, data = Nfull))
qqnorm(resid(mod1.tree))
qqline(resid(mod1.tree))
