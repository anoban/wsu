...
#define lg_x_n (R_FINITE(x / np) ? log(x / np) : (log(x) - log(np)))
    return (x > np) ? x * (lg_x_n - 1.) + np : x * lg_x_n + np - x;
#undef lg_x_n
}
...
static inline long double __stdcall dnorm(long double x, long double mu, long double sigma, int give_log) noexcept {
#ifdef IEEE_754
    ...
    if (!::isfinite(sigma)) return 0;
    if (!::isfinite(x) && mu == x) return NAN; /* x-mu is NaN */
    if (sigma == 0) return (x == mu) ? INF : 0;
    x = (x - mu) / sigma;
    ...
    return INV_SQRT_2PI * exp(-0.5 * x * x) / sigma;
#else
    // more accurate, less fast :
    if (x < 5) return INV_SQRT_2PI * exp(-0.5 * x * x) / sigma;
     ...
     */
    long double x1 = //  nearbyintl(x * 65536) / 65536 =
        ::ldexp(nearbyintl(::ldexp(x, 16)), -16);
    long double x2 = x - x1;
    return INV_SQRT_2PI / sigma * (exp(-0.5 * x1 * x1) * exp((-0.5 * x2 - x1) * x2));
#endif
}

static inline long double __stdcall dt(long double x, long double n, bool logtrans) noexcept {
#ifdef IEEE_754
    ...
    if (!::isfinite(x)) return 0;
    if (!::isfinite(n)) return dnorm(x, 0., 1., logtrans);
    ...
    return exp(t - u) * INV_SQRT_2PI * I_sqrt_;
}
...
static inline long double df(long double x, long double m, long double n, int give_log) noexcept {
    ...
    if (x < 0.) return 0;
    if (x == 0.) return m > 2 ? 0 : (m == 2 ? R_D__1 : INF);
