...
void NodeCharF(phylo P, traits T, nodes N, int* PTattach) {
    ...
    for (n = 0; n < P.nnodes; n++) {
        ...
        //=======
        {
            ...
            // Internals
            {
                N.nCh[node][t][0]  = tmp[node][t][0] / tmp[node][t][1];
                //if (t == 0)
                //{
                //if (NoBL == 0)
                //>>>>>>> 3.5
                ...
                if (node != 0) {
                    tmp[upn][t][0] += N.nCh[node][t][0] / tbl;
                    tmp[upn][t][1] += 1 / tbl;
                }
            }
        }
...
void TipStats(phylo P, traits T, nodes N, int* PTattach) {
    ...
    if (FALSE) {
        for (node = 0; node < P.nnodes; node++)
            for (x = 0; x < T.ntraits; x++) printf("%d\t%d\t%f\t%f\n", node, x, N.tCh[node][x][0], N.tCh[node][x][1]);
    }
    // To HERE
}
    ...
    for (node = 0; node < nnodes; node++)
        for (t = 0; t < ntraits; t++)
            for (i = 0; i < 2; i++) {
                ...
                if (N0.tCh[node][t][i] <= N.tCh[node][t][i]) N0.tChLSig[node][t][i]++;
                if (N0.tCh[node][t][i] >= N.tCh[node][t][i]) N0.tChHSig[node][t][i]++;
            }
}
...
void PIC(phylo P, nodes N) {
    ...
    for (node = 0; node < P.nnodes; node++) upn = 0;
}
    ...
    for (n = 0; n < P.nnodes; n++) {
        ...
        if (P.noat[node] != 0) {
            for (t = 0; t < T.ntraits; t++) {
                ...
                for (i = 0; i < 2; i++)
                    printf("%#1.3f\t%#4.f\t%#4.f\t", N0.tCh[node][t][i], N0.tChLSig[node][t][i], N0.tChHSig[node][t][i]);
                printf("\n");
            }
        }
    }
