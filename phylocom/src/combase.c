...
void SimpleDist(phylo Intree) {
    ...
    for (i = 0; i < Intree.nnodes; i++)
        if (Intree.noat[i] == 0) printf("\t%s", Intree.taxon[i]);
    printf("\n");
    ...
    for (i = 0; i < Intree.nnodes; i++) {
        if (Intree.noat[i] == 0) {
            ...
            for (j = 0; j < Intree.nnodes; j++)
                if (Intree.noat[j] == 0) printf("\t%f", Intree.dist[i][j]);
            printf("\n");
        }
    }
}
...
void AttachSampleToPhylo(struct sample S, struct phylo P, int* SPattach) {
    ...
    for (i = 0; i < S.ntaxa; i++) {
        ...
        if (found == 0) {
            printf("Taxon %s in sample not a terminal taxon in phylogeny. Exiting.\n", S.taxa[i]);
            exit(EXIT_FAILURE);
        }
        if (Debug) printf("Sid %d\tPid %d\tSn %s\tPn %s\n", i, SPattach[i], S.taxa[i], P.taxalist[SPattach[i]]);
    }
}
...
void AttachPhyloToTraits(struct phylo P, struct traits T, int* PTattach) {
    ...
    for (i = 0; i < T.ntaxa; i++) {
        ...
        if (matched == 0) {
            printf("  Taxa string `%s' in traits file not found in phylo file.\n", T.taxon[i]);
            printf("  Exiting.\n");
            exit(EXIT_FAILURE);
        }
    }
...
void DistMatrixNN(struct phylo P) {
    ...
    for (i = 0; i < P.nnodes; i++) {
        for (j = 0; j < P.nnodes; j++) {
            ...
            P.dist[i][j] = cumulativeDist;
            if (Debug) printf("nodes %d %s,%d %s:\tdistance %f\n", i, P.taxon[i], j, P.taxon[j], cumulativeDist);
        }
    }
}
...
int FindMRCA(phylo P, int xNode, int yNode) {
    ...
    while (xNode != yNode) {
        xDepth = P.depth[xNode];
        yDepth = P.depth[yNode];
        if (xDepth >= yDepth) xNode = P.up[xNode];
        if (yDepth >= xDepth) yNode = P.up[yNode];
    }
...
void ComStruct(phylo InP, sample InS, int SwapMethod, int UseAbund) {
    ...
    if (SwapMethod == 3 || SwapMethod == 4) {
        printf(
            "Phylocom output: randomization method %d, %d swaps/trials,  %ld burnin swaps/trials, %d runs\n",
            SwapMethod,
            SWAPS,
            BURNIN,
            RUNS
    ...
    if (Verbose) printf("Run\tSample\tMPD\tMNTD\n");
    ...
    for (runNum = 0; runNum < RUNS; runNum++) {
        ...
        for (plot = 0; plot < InS.nsamples; plot++) {
            ...
            if (NR_plot >= meanX[plot]) pLowCountMeanX[plot]++;
            if (NR_plot <= meanX[plot]) pHighCountMeanX[plot]++;
            ...
            if (NT_plot >= meanXMin[plot]) pLowCountMeanXMin[plot]++;
            if (NT_plot <= meanXMin[plot]) pHighCountMeanXMin[plot]++;

            //if Verbose output desired, write raw random MPD/MNTD
            if (Verbose) printf("%d\t%s\t%f\t%f\n", runNum, InS.pname[plot], NR_plot, NT_plot);
        }
    }
    ...
    for (plot = 0; plot < InS.nsamples; plot++) {
        ...
        for (runNum = 0; runNum < RUNS; runNum++) {
            randomized_sdMeanX[plot]    += pow(randomized_meanX[plot] - randomized_plotMeanX[plot][runNum], 2) / (double) (RUNS - 1);
            randomized_sdMeanXMin[plot] += pow(randomized_meanXMin[plot] - randomized_plotMeanXMin[plot][runNum], 2) / (double) (RUNS - 1);
        }
    ...
    for (plot = 0; plot < InS.nsamples; plot++) {
        ...
        plot_NRI[plot] = (double) (-1.0) * ((meanX[plot] - randomized_meanX[plot]) / randomized_sdMeanX[plot]);
        plot_NTI[plot] = (double) (-1.0) * ((meanXMin[plot] - randomized_meanXMin[plot]) / randomized_sdMeanXMin[plot]);
        ...
        printf(
            ...
            InS.srec[plot],
            meanX[plot],
            randomized_meanX[plot],
            randomized_sdMeanX[plot],
            plot_NRI[plot],
            pLowCountMeanX[plot],
            pHighCountMeanX[plot],
            meanXMin[plot],
            randomized_meanXMin[plot],
            randomized_sdMeanXMin[plot],
            plot_NTI[plot],
...
void IndependentSwap(sample S, int NumberOfSwaps) {
    ...
    for (swapNum = 0; swapNum < NumberOfSwaps; swapNum++) {
        ...
        while (swapped == FALSE) {
            ...
            randomTaxon1 = rand() % S.ntaxa;
            randomTaxon2 = rand() % S.ntaxa;
            while (randomTaxon1 == randomTaxon2) randomTaxon2 = ((int) (((double) S.ntaxa * rand()) / (double) (RAND_MAX)));
            randomPlot1 = rand() % S.nsamples;
            randomPlot2 = rand() % S.nsamples;
            while (randomPlot1 == randomPlot2) randomPlot2 = rand() % S.nsamples;
            ...
            if (SppOnePresentInPlotOne == SppTwoPresentInPlotTwo) {
                if (SppOnePresentInPlotTwo == SppTwoPresentInPlotOne) {
                    if (SppOnePresentInPlotOne != SppTwoPresentInPlotOne) {
                        ...
                        S.id[sppOnePlot][sppOneIndiv]    = S.id[sppTwoPlot][sppTwoIndiv];
                        S.id[sppTwoPlot][sppTwoIndiv]    = idHolder;
                        abundHolder                      = S.abund[sppOnePlot][sppOneIndiv];
                        S.abund[sppOnePlot][sppOneIndiv] = S.abund[sppTwoPlot][sppTwoIndiv];
...
void PhylogenySampleTaxaShuffle(phylo P, sample S, int* attach) {
    ...
    for (taxon = 0; taxon < S.ntaxa; taxon++) {
        ...
        while (foundTaxon == FALSE) {
            ...
            randomSampleTaxon = rand() % S.ntaxa;
            ...
            if (taxon != randomSampleTaxon) {
                ...
                strcpy(P.taxalist[oldAttachTaxon], P.taxalist[oldAttachRandomSampleTaxon]);
                strcpy(P.taxon[P.t2n[oldAttachTaxon]], P.taxon[P.t2n[oldAttachRandomSampleTaxon]]);
                strcpy(P.taxalist[oldAttachRandomSampleTaxon], tempTaxonName);
                strcpy(P.taxon[P.t2n[oldAttachRandomSampleTaxon]], tempTaxonName);
...
void PhylogenyAttachShuffle(phylo P, sample S, int* attach) {
    ...
    for (taxon = 0; taxon < P.termtaxa; taxon++) taxaList[taxon] = taxon;
    ...
    for (taxon = 0; taxon < P.termtaxa; taxon++) {
        randomPhyloTaxon           = rand() % P.termtaxa;
        tmp                        = taxaList[taxon];
    ...
    for (taxon = 0; taxon < S.ntaxa; taxon++) {
        ...
        if (Debug) printf("%d\t", attach[taxon]);
    }
    if (Debug) printf("\n");
}
...
void RandomizeSampleTaxaShuffle(sample S) {
    ...
    for (taxon = 0; taxon < S.ntaxa; taxon++) taxaList[taxon] = taxon;
    ...
    if (Debug) {
        for (plot = 0; plot < InS.nsamples; plot++)
            for (taxon = 0; taxon < InS.srec[plot]; taxon++)
                printf("%s\t%d\t%s\n", InS.pname[plot], InS.abund[plot][taxon], InP.taxalist[attach[InS.id[plot][taxon]]]);
    }
...
double MeanDistance(phylo InP, sample InS, int* attach, int plot, int abundWeighted) {
    if (abundWeighted == 1) {
        ...
        for (j = 0; j < InS.srec[plot]; j++) {
            for (k = 0; k < InS.srec[plot]; k++) {
                ...
                distSum +=
                    InS.abund[plot][j] * InS.abund[plot][k] * InP.dist[InP.t2n[attach[InS.id[plot][j]]]][InP.t2n[attach[InS.id[plot][k]]]];
        ...
        for (j = 0; j < InS.srec[plot] - 1; j++) {
            for (k = j + 1; k < InS.srec[plot]; k++) {
                ...
                plotSum += InP.dist[InP.t2n[attach[InS.id[plot][j]]]][InP.t2n[attach[InS.id[plot][k]]]];
                pairCount++;
            }
        }
        //calculate mean pairwise distance among taxa for this plot
        if (Debug) printf("%f\t%ld\n", plotSum, pairCount);
...
double MeanMinimumDistance(phylo InP, sample InS, int* attach, int plot, int abundWeighted) {
    int    j, k;
    double thisDist   = 0.0;
    double minDist    = 0.0;
        ...
        for (j = 0; j < InS.srec[plot]; j++) {
            ...
            for (k = 0; k < InS.srec[plot]; k++) {
                if (k != j) {
                    ...
                    thisDist = InP.dist[InP.t2n[attach[InS.id[plot][j]]]][InP.t2n[attach[InS.id[plot][k]]]];
...
void CommunityDistanceNull(phylo InP, sample InS, int SwapMethod, int abundWeighted, int nullTest) {
            ...
            for (plot2 = 0; plot2 < InS.nsamples; plot2++) {
                ...
                for (j = 0; j < InS.srec[plot1]; j++) {
                    for (k = 0; k < InS.srec[plot2]; k++) {
                        ...
                        distSum  += InS.pabund[plot1][j] * InS.pabund[plot2][k] * TaxaDist(InP, InS.id[plot1][j], InS.id[plot2][k], attach);
        ...
        for (plot1 = 0; plot1 < InS.nsamples; plot1++) {
            for (plot2 = 0; plot2 < InS.nsamples; plot2++) {
                ...
                for (j = 0; j < InS.srec[plot1]; j++) {
                    for (k = 0; k < InS.srec[plot2]; k++) {
                        //Sum distances among all pairs of taxa in the 2 plots
                        plotSum += TaxaDist(InP, InS.id[plot1][j], InS.id[plot2][k], attach);
                        pairCount++;
                    }
                }
                //calculate weighted mean pairwise distance among taxa
    ...
    if (nullTest == 1) {
        ...
        for (runNum = 0; runNum < RUNS; runNum++) {
            //initialize matrices
            for (plot1 = 0; plot1 < InS.nsamples; plot1++)
                for (plot2 = 0; plot2 < InS.nsamples; plot2++) comDistRnd[runNum][plot1][plot2] = 0.0;
                    ...
                    for (plot2 = 0; plot2 < InS.nsamples; plot2++) {
                        ...
                        for (j = 0; j < InS.srec[plot1]; j++) {
                            for (k = 0; k < InS.srec[plot2]; k++) {
                                //Sum distances among all pairs of taxa in the 2 plots
                                distSum +=
                                    InS.pabund[plot1][j] * InS.pabund[plot2][k] * TaxaDist(InP, InS.id[plot1][j], InS.id[plot2][k], attach);
                                abundSum += InS.pabund[plot1][j] * InS.pabund[plot2][k];
                            }
                        }
                        //calculate weighted mean pairwise distance among taxa
        ...
        for (plot1 = 0; plot1 < InS.nsamples; plot1++) {
            for (plot2 = 0; plot2 < InS.nsamples; plot2++) {
                ...
                for (runNum = 0; runNum < RUNS; runNum++) {
                    //sum squares
                    comDistRndSD[plot1][plot2] +=
                        pow(comDistRndMean[plot1][plot2] - comDistRnd[runNum][plot1][plot2], 2) / (double) (RUNS - 1);
                }
    ...
    //summarize results
    if (nullTest == 1) printf("#ObservedComdist\n");
    printf(".");
    for (plot1 = 0; plot1 < InS.nsamples; plot1++) printf("\t%s", InS.pname[plot1]);
    printf("\n");
    for (plot1 = 0; plot1 < InS.nsamples; plot1++) {
        printf("%s", InS.pname[plot1]);
        for (plot2 = 0; plot2 < InS.nsamples; plot2++) printf("\t%f", comDist[plot1][plot2]);
        printf("\n");
    }
    ...
    if (nullTest == 1) {
        ...
        for (plot1 = 0; plot1 < InS.nsamples; plot1++) {
            printf("%s", InS.pname[plot1]);
            for (plot2 = 0; plot2 < InS.nsamples; plot2++) printf("\t%f", comDistRndMean[plot1][plot2]);
            printf("\n");
        }
        ...
        for (plot1 = 0; plot1 < InS.nsamples; plot1++) {
            printf("%s", InS.pname[plot1]);
            for (plot2 = 0; plot2 < InS.nsamples; plot2++) printf("\t%f", comDistRndSD[plot1][plot2]);
            printf("\n");
        }
...
void CommunityDistanceNNNull(phylo InP, sample InS, int SwapMethod, int abundWeighted, int nullTest) {
    ...
    for (plot1 = 0; plot1 < InS.nsamples; plot1++) {
        for (plot2 = 0; plot2 < InS.nsamples; plot2++) {
            if (plot1 != plot2) {
                ...
                comDist[plot1][plot2] = MeanMinimumDistance(InP, InS, attach, plot1, abundWeighted);
        }
    }
    ...
    //summarize results
    if (nullTest == 1) printf("#ObservedComdistNT\n");
    printf(".");
    for (plot1 = 0; plot1 < InS.nsamples; plot1++) printf("\t%s", InS.pname[plot1]);
    printf("\n");
    for (plot1 = 0; plot1 < InS.nsamples; plot1++) {
...
void PhyloDiversity(phylo P, sample S) {
    ...
    for (plot = 0; plot < S.nsamples; plot++) {
        ...
        for (i = 0; i < S.srec[plot]; i++) {
            for (j = 0; j < S.srec[plot]; j++) {
                //if (i != j) {
                Dp[plot] += (TaxaDist(P, S.id[plot][i], S.id[plot][j], attach) / 2.0) * S.pabund[plot][i] * S.pabund[plot][j];
                if (i != j) {
                    Di[plot]     += S.pabund[plot][i] * S.pabund[plot][j];
                    DeltaP[plot] += (TaxaDist(P, S.id[plot][i], S.id[plot][j], attach) / 2.0);
                    countDist++;
                }
                //}
            }
        }
        DiHat[plot]   = Di[plot] * ((double) S.irec[plot] / ((double) S.irec[plot] - 1.0));
        DpHat[plot]   = Dp[plot] * ((double) S.irec[plot] / ((double) S.irec[plot] - 1.0));
                ...
                for (i = 0; i < S.srec[k]; i++) {
                    for (j = 0; j < S.srec[l]; j++) {
                        if (S.id[k][i] != S.id[l][j]) {
                            ...
                            DpIJ[k][l]     += S.pabund[k][i] * S.pabund[l][j] * (TaxaDist(P, S.id[k][i], S.id[l][j], attach) / 2.0);
                            DeltaPIJ[k][l] += (TaxaDist(P, S.id[k][i], S.id[l][j], attach) / 2.0);
                            countDiff++;
                        }
                    }
                }
    ...
    for (i = 0; i < S.ntaxa; i++) {
        for (j = 0; j < S.ntaxa; j++) {
            if (i != j) {
                ...
                DpT     += (TaxaDist(P, i, j, attach) / 2.0) * S.psppabund[i] * S.psppabund[j];
    ...
    for (i = 0; i < S.ntaxa; i++) {
        for (j = 0; j < S.ntaxa; j++) {
            if (i != j) {
                ...
                DpTHat += (TaxaDist(P, i, j, attach) / 2.0) * S.psppabund[i] * S.psppabund[j];
            }
        }
    }
        ...
        for (l = 0; l < S.nsamples; l++) {
            if (k != l) {
                ...
                for (i = 0; i < S.srec[k]; i++) {
                    for (j = 0; j < S.srec[l]; j++) {
                        if (i != j) {
                            sampDist += (TaxaDist(P, S.id[k][i], S.id[l][j], attach) / 2.0);
                            sampCount++;
                        }
                    }
                }
    ...
    printf("#----- AMONG - Di.kl\n");
    printf(".");
    for (k = 0; k < S.nsamples; k++) printf("\t%s", S.pname[k]);
    printf("\n");
    for (k = 0; k < S.nsamples; k++) {
        printf("%s", S.pname[k]);
        for (l = 0; l < S.nsamples; l++) printf("\t%f", DiIJ[k][l]);
        printf("\n");
    }
    printf("#----- AMONG - Dp.kl\n");
    printf(".");
    ...
    for (k = 0; k < S.nsamples; k++) {
        printf("%s", S.pname[k]);
        for (l = 0; l < S.nsamples; l++) printf("\t%f", DpIJ[k][l]);
        printf("\n");
    }
    ...
    for (k = 0; k < S.nsamples; k++) {
        printf("%s", S.pname[k]);
        for (l = 0; l < S.nsamples; l++) printf("\t%f", DeltaPIJ[k][l]);
        printf("\n");
    }
...
void RaoDiversity(phylo P, sample S) {
    ...
    for (k = 0; k < S.nsamples - 1; k++) {
        for (l = k + 1; l < S.nsamples; l++) {
            ...
            for (i = 0; i < S.srec[k]; i++) {
                for (j = 0; j < S.srec[l]; j++) {
                    if (S.id[k][i] != S.id[l][j]) H[k][l] += S.pabund[k][i] * S.pabund[l][j];
                    Hp[k][l] += (TaxaDist(P, S.id[k][i], S.id[l][j], attach) / 2.0) * S.pabund[k][i] * S.pabund[l][j];
                }
            }
    ...
    for (i = 0; i < S.ntaxa; i++) {
        for (j = 0; j < S.ntaxa; j++) {
            if (i != j) {
                ...
                Dtotalp += ((double) TaxaDist(P, i, j, attach) / 2.0) * (double) S.psppabund[i] * (double) S.psppabund[j];
            }
            sumDist += (TaxaDist(P, i, j, attach) / 2.0);
            countDist++;
        }
    ...
    for (k = 0; k < S.nsamples; k++) {
        for (l = 0; l < S.nsamples; l++) {
            if (k != l) {
                Dbeta  += H[k][l] * ((float) S.irec[k] / (float) S.totabund) * ((float) S.irec[l] / (float) S.totabund);
                Dbetap += Hp[k][l] * ((float) S.irec[k] / (float) S.totabund) * ((float) S.irec[l] / (float) S.totabund);
            }
        }
    }
    ...
    if (S.nsamples > 0) {
        for (i = 1; i < S.nsamples; i++) {
            if (S.irec[i] != S.irec[i - 1]) {
                printf(
                    "#Warning - diversity components are only interpretable when all communities contain equal numbers of individuals. See manual for details.\n"
                );
                break;
            }
        }
    ...
    for (plot = 0; plot < S.nsamples; plot++) {
        printf(
            ...
            S.srec[plot],
            S.irec[plot],
            (float) S.irec[plot] / (float) S.totabund,
            D[plot],
            Dp[plot]
        );
    }
