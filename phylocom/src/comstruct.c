...
void PD(struct phylo P, struct sample S) {
    ...
    for (i = 0; i < S.nsamples; i++) {
        ...
        printf("%s\t%-d\t%-7.3f\t%-7.3f\t%-7.3f\n", S.pname[i], S.srec[i], sampletotbl[i], totalbl, sampletotbl[i] / totalbl);
    }
}
...
void ComDist(struct phylo P, struct sample S) {
    ...
    for (i = 0; i < S.nsamples; i++)
        for (j = 0; j < S.nsamples; j++) mdist[i][j] = 0.0;
    ...
    // headers
    printf(".");
    for (i = 0; i < S.nsamples; i++) printf("\t%s", S.pname[i]);
    printf("\n");
    ...
    for (i = 0; i < S.nsamples; i++) {
        ...
        for (j = 0; j < S.nsamples; j++) printf("\t%f", mdist[j][i]);
        printf("\n");
    }
}
    ...
    for (p = 0; p < S.nsamples; p++) {
        for (q = p + 1; q < S.nsamples; q++) {
            ...
            for (i = 0; i < S.srec[p]; i++) {
                ...
                for (j = 0; j < S.srec[q]; j++) {
                    rel = P.dist[P.t2n[attach[S.id[p][i]]]][P.t2n[attach[S.id[q][j]]]];
    ...
    for (p = 0; p < S.nsamples; p++) {
        ...
        for (i = 0; i < S.srec[p]; i++) {
            ...
            printf(
                ...
                S.taxa[S.id[p][i]],
                (totR / (float) comb),
                (-1 * ((totR / (float) comb) - InM.mpd[S.srec[p]]) / InM.sdpd[S.srec[p]])
            );
        }
    }
    free_matrix(P.dist, 0, P.nnodes - 1, 0, P.nnodes - 1);
    ...
    for (p = 0; p < S.nsamples; p++) {
        ...
        for (i = 0; i < S.srec[p]; i++) {
            ...
            printf(
                ...
                S.taxa[S.id[p][i]],
                minR,
                (-1 * (minR - InM.mnnd[S.srec[p]]) / InM.sdnnd[S.srec[p]])
            );
        }
    }
    free_matrix(P.dist, 0, P.nnodes - 1, 0, P.nnodes - 1);
...
void VMeans(struct phylo P) {
    ...
    termvect = ivector(0, P.termtaxa - 1);
    j        = 0;
    ...
    for (n = 2; n < P.termtaxa; n++) {
        ...
        for (r = 0; r < 1000; r++) {
            ...
            while (found < n) {
                rnd = (int) (((float) P.termtaxa * (float) rand()) / (float) (RAND_MAX + 1.0));
                if (sudah[rnd] == 0) {
        ...
        var    = (mpdTotSqr - ((mpdTot * mpdTot) / (float) mpdTotComb)) / (float) (mpdTotComb - 1);

        // nn
        NNmean = nndTot / (float) nndTotComb;
        NNvar  = (nndTotSqr - ((nndTot * nndTot) / (float) nndTotComb)) / (float) (nndTotComb - 1);
...
void Ltt(struct phylo P, struct sample S) {
    ...
    for (i = 0; i < P.termtaxa; i++) {
        ...
        while (xnode != 0) {
            runningd += P.bl[xnode];
            if ((runningd > (r2t * 0.25)) && (orunningd <= (r2t * 0.25))) deep1[xnode] = 1;
            if ((runningd > (r2t * 0.5)) && (orunningd <= (r2t * 0.5))) deep2[xnode] = 1;
            if ((runningd > (r2t * 0.75)) && (orunningd <= (r2t * 0.75))) deep3[xnode] = 1;
            xnode     = P.up[xnode];
            orunningd = runningd;
        }
    }
    ...
    printf(
        "all     \t%d\t%5.3f\t%5.3f\t%5.3f\t%5.3f\t%5.3f\n",
        P.termtaxa,
        1.0 / (float) P.termtaxa,
        (float) ltt1 / (float) P.termtaxa,
        (float) ltt2 / (float) P.termtaxa,
        (float) ltt3 / (float) P.termtaxa,
        1.0
    );
    ...
    for (j = 0; j < S.nsamples; j++) {
        ...
        // Output
        printf(
            "%-8s\t%d\t%5.3f\t%5.3f\t%5.3f\t%5.3f\t%5.3f\n",
            S.pname[j],
            S.srec[j],
            1.0 / (float) P.termtaxa,
            (float) ltt1 / (float) S.srec[j],
            (float) ltt2 / (float) S.srec[j],
            (float) ltt3 / (float) S.srec[j],
            1.0
        );
    }
}
...
void LttR(struct phylo P, struct sample S) {
    ...
    for (i = 0; i < P.termtaxa; i++) {
        ...
        while (xnode != 0) {
            runningd += P.bl[xnode];
            if ((runningd > (r2t * 0.1)) && (orunningd <= (r2t * 0.1))) deep1[xnode] = 1;
            if ((runningd > (r2t * 0.2)) && (orunningd <= (r2t * 0.2))) deep2[xnode] = 1;
            if ((runningd > (r2t * 0.3)) && (orunningd <= (r2t * 0.3))) deep3[xnode] = 1;
            if ((runningd > (r2t * 0.4)) && (orunningd <= (r2t * 0.4))) deep4[xnode] = 1;
            if ((runningd > (r2t * 0.5)) && (orunningd <= (r2t * 0.5))) deep5[xnode] = 1;
            if ((runningd > (r2t * 0.75)) && (orunningd <= (r2t * 0.75))) deep75[xnode] = 1;
            xnode     = P.up[xnode];
            orunningd = runningd;
        }
    }
    ...
    printf(
        "all       %4d\t%5.3f\t%5.3f\t%5.3f\t%5.3f\t%5.3f\t%5.3f\t%5.3f\t%5.3f\n",
        P.termtaxa,
        1.0 / (float) P.termtaxa,
        (float) ltt75 / (float) P.termtaxa,
        (float) ltt5 / (float) P.termtaxa,
        (float) ltt4 / (float) P.termtaxa,
        (float) ltt3 / (float) P.termtaxa,
        (float) ltt2 / (float) P.termtaxa,
        (float) ltt1 / (float) P.termtaxa,
        1.0
    ...
    for (j = 0; j < S.nsamples; j++) {
        ...
        for (run = 0; run < RUNS; run++) {
            ...
            switch (SWAPMETHOD) {
                    ...
                    //break;
                case 1 : RandomizeSampleTaxaShuffle(S); break;
                case 2 :
        ...
        // Output
        printf(
            "%-8s  %4d\t%5.3f\t%5.3f\t%5.3f\t%5.3f\t%5.3f\t%5.3f\t%5.3f\t%5.3f\n",
            S.pname[j],
            S.srec[j],
            1.0 / (float) S.srec[j],
            (float) ltt75 / (float) S.srec[j],
            (float) ltt5 / (float) S.srec[j],
            (float) ltt4 / (float) S.srec[j],
            (float) ltt3 / (float) S.srec[j],
            (float) ltt2 / (float) S.srec[j],
            (float) ltt1 / (float) S.srec[j],
            1.0
...
void NodeSig(phylo P, sample S, int outmethod, int abundWeighted) {
    ...
    for (i = 0; i < S.nsamples; i++) {
        ...
        if (TreeView == 0) Out[i].arenotes = 1;
        if (TreeView == 1) Out[i].arenotes = 0;

        // dimension a new array for Out names:
        if (TreeView == 0) Out[i].notes = cmatrix(0, P.nnodes - 1, 0, MAXNOTELENGTH + 10);
        if (TreeView == 1) Out[i].taxon = cmatrix(0, P.nnodes - 1, 0, MAXTAXONLENGTH + 10);
    }

    // for each plot

    if (outmethod == 1) printf("plot\tnode\tnode_name           \tntaxa\tmedian\trank\tsig\n");
    for (plot = 0; plot < S.nsamples; plot++) {
        if (S.srec[plot] > 2) {
            for (node = 0; node < P.nnodes; node++) {
                tipsReal_n[node] = 0;
                for (run = 0; run < RUNS; run++) tips_rn[run][node] = 0;
            }
            ...
            for (node = 0; node < P.nnodes; node++) {
                if (TreeView == 0) strcpy(Out[plot].notes[node], "");
                if (TreeView == 1) strcpy(Out[plot].taxon[node], "");
                ...
                if (P.noat[node] != 0) {
                    ...
                    ordLO = 0;
                    for (run = 0; run < RUNS; run++) {
                        if (tips_rn[run][node] < tipsReal_n[node]) ordHI++;
                        if (tips_rn[run][node] > tipsReal_n[node]) ordLO++;
                    }
                    ...
                    if (ordLO >= (int) ((float) RUNS * 0.975)) {
                        strcpy(mark, "-");
                        if (TreeView == 0) strcat(Out[plot].notes[node], "SIGLESS");
                        if (TreeView == 1) {
                        ...
                        strcpy(mark, "+");
                        if (TreeView == 0) strcat(Out[plot].notes[node], "SIGMORE");
                ...
                if ((TreeView == 1) && (strcmp(P.taxon[node], ".") != 0)) strcat(Out[plot].taxon[node], P.taxon[node]);
            }
        }
        // Name tree
    ...
    if (outmethod == 0) WriteNexus(Out, S.nsamples, ReadSample(SampleFile), 1, ReadTraits(TraitFile), 1);
}
