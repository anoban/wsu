...
int main(int argc, char* argv[]) {
    ...
    srand(time(NULL)); // do this once only
    ...
    for (argx = 0; argx < argc; argx++) {
        ...
        // prunes
        if (strcmp(argv[argx], "-p") == 0) sscanf(argv[argx + 1], "%d", &RNDPRUNEN);

        // swaps
        if (strcmp(argv[argx], "-w") == 0) sscanf(argv[argx + 1], "%d", &SWAPS);

        // swaps
        if (strcmp(argv[argx], "-b") == 0) sscanf(argv[argx + 1], "%ld", &BURNIN);

        // swap method/model
        // defaults to 0 (shuffle phylogeny)
        if (strcmp(argv[argx], "-m") == 0) sscanf(argv[argx + 1], "%d", &SWAPMETHOD);

        // phylo file
        if (strcmp(argv[argx], "-f") == 0) PhyloFile = argv[argx + 1];
        ...
        if (strcmp(argv[argx], "-a") == 0) UseAbund = 1;

        // sample file
        if (strcmp(argv[argx], "-s") == 0) SampleFile = argv[argx + 1];

        // traits file
        if (strcmp(argv[argx], "-t") == 0) TraitFile = argv[argx + 1];

        // Ignore branch lengths in node calculations and taxon distances (traits)
        //TODO implement this for other functions as well
        if (strcmp(argv[argx], "-e") == 0) NoBL = 1;

        if (strcmp(argv[argx], "-y") == 0) FYOUT = 1;

        //Debug
        if (strcmp(argv[argx], "-d") == 0) Debug = 1;

        //Verbose - switch to toggle verbose output
        if (strcmp(argv[argx], "-v") == 0) Verbose = 1;

        // Select x variable for contrasts (default = 0)
        if (strcmp(argv[argx], "-x") == 0) sscanf(argv[argx + 1], "%d", &XVAR);
        // Subset aot output (default = 0 = all)
        if (strcmp(argv[argx], "-o") == 0) sscanf(argv[argx + 1], "%d", &AOTOUT);
    }
    ...
    // Simple new2fy conversion
        ...
        WriteNexus(IntreeV, 1, ReadSample(SampleFile), 1, ReadTraits(TraitFile), 1);
    ...
    else if ((strcmp(Method, "comdistnn") == 0) || (strcmp(Method, "comdistnt") == 0)) {
        // ComDist(ReadPhylogeny(PhyloFile), ReadSample(SampleFile));
