...
int CleanPhy(phylo P) {
    ...
    for (i = 0; i < P.nnodes; i++) {
        ...
        if ((P.ldown[i] != -99) && (P.up[i] != -99) && (P.rsister[P.ldown[i]] == -99)) {
            // printf("fixing %d\n", i);
            at = i;
            ...
            while (P.rsister[P.ldown[at]] == -99) {
                ...
                // if right branch
                    ...
                    while (P.rsister[mark] != at) mark = P.rsister[mark];
                    P.rsister[mark] = P.ldown[at];
                }
                // or if with another empty node above
                else if ((P.rsister[at] == -99) && (P.rsister[P.ldown[P.up[at]]] == -99)) {
                    // nothing needed
                }
                // fix daughter -> parent
                P.up[P.ldown[at]] = P.up[at];
                // fix parent -> daughter
                if (P.ldown[P.up[at]] == at) P.ldown[P.up[at]] = P.ldown[at];
