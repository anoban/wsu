...
int main(int argc, char* argv[]) {
    ...
    // srandom(time(NULL));
    //srandom(2);
    srand(getpid()); // need this because in a shell-script the program
    ...
    for (argx = 0; argx < argc; argx++) {
        // runs
        if (strcmp(argv[argx], "-s") == 0) sscanf(argv[argx + 1], "%f", &p_SPECIATE);
        if (strcmp(argv[argx], "-e") == 0) sscanf(argv[argx + 1], "%f", &p_EXTINCT);
        if (strcmp(argv[argx], "-t") == 0) sscanf(argv[argx + 1], "%d", &MAX_TIME);
        if (strcmp(argv[argx], "-d") == 0) {
            TAPER = 1;
            sscanf(argv[argx + 1], "%d", &TAPERFACT);
        }
        if (strcmp(argv[argx], "-m") == 0) sscanf(argv[argx + 1], "%d", &OUT_MODE);
        if (strcmp(argv[argx], "-c") == 0) sscanf(argv[argx + 1], "%s", CHANGE);
        if (strcmp(argv[argx], "-p") == 0) PRUNED = 1;
        if (strcmp(argv[argx], "-l") == 0) {
            strcpy(STOPPER, "lineages");
            sscanf(argv[argx + 1], "%d", &LINEAGES);
        }

        if (strcmp(argv[argx], "-x") == 0) COMPETE = 1;
    ...
    while (stop == 0) {
        ...
        for (l = 0; l <= ll; l++) {
            if (Active_l[l] == 1) {
                ...
                if (COMPETE) {
                    ...
                    newp_EXTINCT = p_EXTINCT * (5.0 / ((float) EcoDist_l[l] + 1.0));
                    // printf("%f\n",newp_EXTINCT);
                }
                // ^^^                               ^^^

                if (OUT_MODE == 0) printf("      t %d\tl %d\t\t\t\t\tc %f\te %d\n", t, l, Char_ltc[l][t][0], EcoDist_l[l]);

                // check for speciation
                if (((float) rand() / (float) (RAND_MAX + 1.0)) < p_SPECIATE) {
                    Speciate(l, t);
                    alive++;
                ...
                else if ((((float) rand() / (float) (RAND_MAX + 1.0)) < newp_EXTINCT) && (Lineage != 0)) {
                    Extinct(l, t);
                    alive--;
                } else {
                    // Transfer lineage variables to next time
                    for (chi = 0; chi < NCHAR; chi++) Char_ltc[l][t + 1][chi] = Char_ltc[l][t][chi];
        ...
        t++;
        if (((t == MAX_TIME) && (strcmp(STOPPER, "time") == 0)) || ((alive > LINEAGES) && (strcmp(STOPPER, "lineages") == 0)) ||
            (t > BAILT))
            stop = 1;
    }
    ...
    for (l = 0; l <= Lineage; l++) {
        if (Extant_l[l] == 1) {
            ...
            for (t = MAX_TIME; t >= 0; t--) {
                ...
                if (Living_lt[lroute][t - 1] == 0) lroute = LineageUp_l[lroute];
            }
        }
    }
    ...
    for (t = 0; t <= MAX_TIME; t++) {
        ...
        for (chi = 0; chi < NCHAR; chi++) {
            CharSumXSqr[chi]  = (CharSumXSqr[chi] - ((CharMean[chi] * CharMean[chi]) / (float) LttZ)) / (float) (LttZ - 1);
            CharMean[chi]     = CharMean[chi] / (float) LttZ;

            CharCSumXSqr[chi] = (CharCSumXSqr[chi] - ((CharCMean[chi] * CharCMean[chi]) / (float) LttC)) / (float) (LttC - 1);
            CharCMean[chi]    = CharCMean[chi] / (float) LttC;
        }
        // output LTT
        if (OUT_MODE == 2)
            printf("%d\t%d\t%6.1f\t%6.1f\t%d\t%6.1f\t%6.1f\n", t, LttZ, CharMean[0], CharSumXSqr[0], LttC, CharCMean[0], CharCSumXSqr[0]);
...
void Speciate(int l, int t) {
    ...
    // The Char value?
    for (chi = 0; chi < NCHAR; chi++) Char_nc[Node][chi] = Char_ltc[l][t][chi];
    ...
    for (x = 0; x <= 1; x++) {
        ...
        for (chi = 0; chi < NCHAR; chi++) Char_ltc[Lineage][t + 1][chi] = Char_ltc[l][t][chi];
    }
...
void CharChange(int l, int t) {
    ...
    // change = (int) ((17.0 * (float) rand()) /
    //          (float) (RAND_MAX+1.0));

    for (i = 0; i < NCHAR; i++) {
        change = (int) (((double) charch.n * (double) rand()) / (double) (RAND_MAX + 1.0));

        // printf("change = %d, n = %d\n", change, charch.n);
        //if (OUT_MODE == 0) printf("change = %d\n", change);

        if (TAPER) {
            Char_ltc[l][t][i] = Char_ltc[l][t][i] + (charch.x[change] * exp((-1.0 * (float) t) / TAPERFACT));
...
void Output(int t) {
    if (OUT_MODE == 0)
        printf("EVENT t %d\t\tn %d\tu %d\tb %d\t' %d\tc %f\n", t, Node, NodeUp_n[Node], BlUp_n[Node], Name_n[Node], Char_nc[Node][0]);

    if (OUT_MODE == 1) printf("%d\t%d\t%d\t%d\t%f\t%d\n", Node, NodeUp_n[Node], BlUp_n[Node], Name_n[Node], Char_nc[Node][0], Time_n[Node]);
}
...
void MakePhylo() {
    ...
    if (PRUNED == 1)
        Fy2newRec(Prune(OutTree, prunex));
    else
        Fy2newRec(OutTree);
...
void WriteTraits() {
    ...
    for (chi = 0; chi < NCHAR; chi++) fprintf(out_file, "\t3");
    fprintf(out_file, "\n");
    fprintf(out_file, "name");
    for (chi = 0; chi < NCHAR; chi++) fprintf(out_file, "\tecov%d", chi);
    fprintf(out_file, "\n");
    ...
    for (n = 1; n <= Node; n++) {
        if (PRUNED == 1) {
            if (Terminal_n[n] == 1) {
                ...
                for (chi = 0; chi < NCHAR; chi++) fprintf(out_file, "\t%f", Char_nc[n][chi]);
                fprintf(out_file, "\n");
            }
        } else if (Terminal_n[n] != 0) {
            fprintf(out_file, "%s", OutTree.taxon[n - 1]);
            for (chi = 0; chi < NCHAR; chi++) fprintf(out_file, "\t%f", Char_nc[n][chi]);
            fprintf(out_file, "\n");
        }
...
void DummySample() {
    ...
    for (n = 0; n < Node; n++)
        if (Terminal_n[n + 1] == 1) fprintf(out_file, "alive\t1\t%s\n", OutTree.taxon[n]);
    fclose(out_file);
}
