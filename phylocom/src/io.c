...
void NAF(phylo P[], sample S, traits T) {
    ...
    printf("Tx\\Dx");
    z = 0;
    for (i = 0; i < T.ntraits; i++) printf("\tT:%d:%s", T.type[i], T.trname[i]);
    for (i = 0; i < S.nsamples; i++) printf("\tS:%s", S.pname[i]);
    for (i = 0; i < P[0].nnodes; i++)
        if (P[0].noat[i] != 0) printf("\tN:%s", P[0].taxon[i]);
...
means ReadMeans(phylo Intree, char meansfile[50]) {
    ...
    while (fgets(line, 500, Fm) != NULL) {
        sscanf(line, "%d\t%10f\t%10f\t%10f\t%10f", &samplesizeI, &meanI, &sdI, &nnmeanI, &nnsdI);
...
void WriteNexus(phylo P[], int ntree, sample S, int nsamp, traits T, int ntrf) {
    ...
    if (nsamp > 0) {
        ...
        for (i = 1; i < S.nsamples; i++) printf(", %d %s", i + 1, S.pname[i]);
        printf(";\n\tMATRIX\n");
        for (i = 0; i < P[0].nnodes; i++) {
            if (P[0].noat[i] == 0) {
                printf("\t%s\t", P[0].taxon[i]);
    ...
    if (ntrf > 0) {
        ...
        if (makedisc > 0) {
            ...
            for (i = 0; i < T.ntaxa; i++) {
                printf("\t%s\t", T.taxon[i]);
                for (j = 0; j < T.ntraits; j++)
                    if ((T.type[j] == 0) || (T.type[j] == 1) || (T.type[j] == 2)) printf("%d", (int) T.tr[i][j]);
                printf("\n");
            }
            printf(";\nEND;\n\n");
        }
        ...
        if (makecont > 0) {
            ...
            for (i = 0; i < T.ntaxa; i++) {
                printf("\t%s\t", T.taxon[i]);
                for (j = 0; j < T.ntraits; j++)
                    if (T.type[j] == 3) printf("  %f", T.tr[i][j]);
                printf("\n");
            }
            printf(";\nEND;\n\n");
        }
            ...
            if (P[0].noat[i] == 0) {
                ...
                if (q == 0) {
                    if (i == P[0].nnodes - 1)
                        ...
                        printf(" %d %s,", j, P[0].taxon[i]);
                }
                sprintf(tmp, "%d", j);
                strcpy(WN[q].taxon[i], tmp);
            } else if ((strcmp(P[q].taxon[i], "") != 0) && (strcmp(P[q].taxon[i], ".") != 0)) {
                strcpy(WN[q].taxon[i], "'");
...
sample ReadSample(char samplefile[50]) {
    ...
    for (i = 0; i < InSample.ntaxa; i++) InSample.sppfreq[i] = 0;
...
traits ReadTraits(char traitfile[50]) {
    ...
    while (myfgets(line, 1000, Cread, lineending) != NULL) {
        ...
        if (nline == 0) {
            ...
            for (j = 1; j < words; j++) C.type[j - 1] = atoi(word[j]);
        }
        ...
        else // lines 2+
        {
            ...
            if ((nline == 1) && (extra == 1)) {
                for (j = 1; j < words; j++) strcpy(C.trname[j - 1], word[j]);
...
void MakeUpPassOrder(phylo P) {
    ...
    if (FALSE)
        for (i = 0; i < P.nnodes; i++) printf("%d\t%d\n", i, P.upo[i]);
}
...
void AssignNodeLists(phylo P) {
    ...
    for (n = 0; n < P.nnodes; n++) {
        ...
        if (P.noat[node] != 0) {
            ...
            intin[node]++;
            if (node != 0) {
                for (i = tipin[P.up[node]]; i < tipin[P.up[node]] + P.ntip[node]; i++) {
                    j                        = i - tipin[P.up[node]];
                    P.tiplist[P.up[node]][i] = P.tiplist[node][j];
                }
                tipin[P.up[node]] += P.ntip[node];

                for (i = intin[P.up[node]]; i < intin[P.up[node]] + P.nint[node]; i++) {
                    j                        = i - intin[P.up[node]];
                    P.intlist[P.up[node]][i] = P.intlist[node][j];
                }
