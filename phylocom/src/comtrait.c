...
void ComTraitMetric(sample S, traits T, int SwapMethod, int XVAR) {
    ...
    for (trait = 0; trait < T.ntraits; trait++) {
        for (samp = 0; samp < S.nsamples; samp++) {
            ...
            //fill the vector
            for (rec = 0; rec < S.srec[samp]; rec++) sampleTraits[rec] = T.tr[STattach[S.id[samp][rec]]][trait];
    ...
    for (trait = 0; trait < T.ntraits; trait++) {
        for (samp = 0; samp < S.nsamples; samp++) {
            printf(
                "%s\t%s\t%d\t%f\t%f\t%f\t%f\t%f\t%d\t%d\t%d\n",
                T.trname[trait],
                S.pname[samp],
                S.srec[samp],
                STMeanObs[samp][trait],
                STMetricObs[samp][trait],
                MeanSTMetricRnd[samp][trait],
                StDevSTMetricRnd[samp][trait],
                SESSTMetricRnd[samp][trait],
                rankLowSTMetricRnd[samp][trait],
                rankHighSTMetricRnd[samp][trait],
                RUNS
            );
        }
    }
...
void TraitsAttachShuffle(sample S, traits T, int* attach) {
    ...
    for (taxon = 0; taxon < S.ntaxa; taxon++) {
        randomTraitsTaxon         = ((int) (((double) T.ntaxa * rand()) / (double) (RAND_MAX)));
...
void traitMetric(float X[], unsigned int n, float* metric, int XVAR) {
    ...
    switch (XVAR) {
        ...
        case 3 :
            ...
            for (i = 0; i < n; i++) minDist = 99999999.9; //needs to be higher than minimum expected distance
            for (j = 0; j < n; j++) {
                if (i != j) {
