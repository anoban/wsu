...
void Fy2new(struct phylo O) {
    ...
    for (i = 0; i < O.nnodes; i++) {
        ...
        if ((strcmp(O.taxon[i], ".") != 0) && (O.noat[i] > 0)) strcpy(InteriorName[i], O.taxon[i]);
    ...
    for (d = MaxDepth; d >= 0; d--) {
            ...
            if (O.depth[i] == d) {
                ...
                for (j = 0; j <= HighestNodeNo; j++) {
                    if (O.depth[j] == d) {
                        ...
                        // above
                        if ((i != j) && (strcmp(ClusterAt[O.up[i]], "") == 0) && (strcmp(ClusterAt[i], "") != 0) &&
                            (strcmp(ClusterAt[j], "") != 0) && (O.up[i] == O.up[j])) {
                            strcpy(ClusterAt[O.up[i]], "(");
                            ...
                            // new bit
                            if (strcmp(InteriorName[O.up[i]], "") != 0) strcat(ClusterAt[O.up[i]], InteriorName[O.up[i]]);

                            strcpy(ClusterAt[O.up[i]], ClusterAt[O.up[i]]);
                            strcat(ClusterAt[O.up[i]], ":");
                            sprintf(blpr, "%f", O.bl[O.up[i]]);
                            strcat(ClusterAt[O.up[i]], blpr);
                            if ((O.arenotes > 0) && (strcmp(O.notes[O.up[i]], "") != 0)) {
                        ...
                        if ((i != j) && (strcmp(ClusterAt[O.up[i]], "") != 0) && (strcmp(ClusterAt[j], "") != 0) && (O.up[i] == O.up[j])) {
                            // find the last )
                            for (qq = strlen(ClusterAt[O.up[i]]); qq > 0; qq--)
                                if (strncmp(&ClusterAt[O.up[i]][qq], ")", 1) == 0) break;
        ...
        //leftovers:
        for (i = 0; i <= HighestNodeNo; i++) {
            if ((O.depth[i] == d) && (strcmp(ClusterAt[i], "") != 0) && (d != 0)) {
    ...
    free_cvector(tmp, 0, (O.nnodes * (MAXBLLENGTH + MAXTAXONLENGTH + MAXNOTELENGTH + 3)));
}
        ...
        if (A.noat[x] == 0) {
            ...
            while (y != 0) {
                ...
                // if not, find the dangling sister
                    ...
                    while (A.rsister[mark] != -99) mark = A.rsister[mark];
                    A.rsister[mark] = y;
                }
...
void Fy2newRec(struct phylo A) {
    char* tmp;
    // printf(">>> %d\n\n", A.nnodes);
    tmp = cvector(0, (A.nnodes * (MAXBLLENGTH + MAXTAXONLENGTH + MAXNOTELENGTH + 3)));
    ...
    printf("%s;\n", downPar(A, 0, tmp));
    free_cvector(tmp, 0, (A.nnodes * (MAXBLLENGTH + MAXTAXONLENGTH + MAXNOTELENGTH + 3)));
}
...
char* downPar(struct phylo A, int atn, char* tmp) {
    ...
    tmpnext = cmatrix(0, A.noat[atn] - 1, 0, (A.nnodes * (MAXBLLENGTH + MAXTAXONLENGTH + MAXNOTELENGTH + 3)));
    ...
    free_cmatrix(tmpnext, 0, A.noat[atn] - 1, 0, (A.nnodes * (MAXBLLENGTH + MAXTAXONLENGTH + MAXNOTELENGTH + 3)));
