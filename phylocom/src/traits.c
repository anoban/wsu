...
void PSigRun(traits T, phylo P, int Output, int xVar, int aotOut) {
    ...
    for (i = 1; i <= T.ntraits - 1; i++) {
        if (t == xVar - 1) t++;
        ordTrt[i] = t;
        if (T.type[t] > 1) inclContrasts = 1;
        t++;
    }

    if (Debug)
        for (k = 0; k <= T.ntraits - 1; k++) printf("%d %d\n", k, ordTrt[k]);
    // exit(EXIT_FAILURE);
    // printf("%d\n",P.nnodes);
    ...
    for (k = 0; k < T.ntraits; k++) {
        t           = ordTrt[k];
        charMean[t] = 0.0;
        for (z = 0; z < T.ntaxa; z++) charMean[t] += T.tr[z][t];
    ...
    for (i = 0; i < P.nnodes; i++)
        if (maxDepth <= P.depth[i]) maxDepth = P.depth[i];

    for (run = 0; run <= RUNS; run++) {
        d = div(run, 50);
        if ((RunNum) && (d.rem == 0)) printf("run %d\n", run);
        ...
        for (depth = maxDepth; depth >= 0; depth--) {
            for (node = 0; node < P.nnodes; node++) {
                ...
                if ((P.depth[node] == depth) && (P.noat[node] == 0)) {
                    P.age[node] = 0;
                    if (P.age[P.up[node]] == -1) P.age[P.up[node]] = P.age[node] + P.bl[node];
                    for (k = 0; k <= T.ntraits - 1; k++) {
                        ...
                        } else if (T.tr[PTattach[node]][0] == 1) {
                            sumTipChar[t][P.up[node]]   += charv;
                            sumsqTipChar[t][P.up[node]] += charv * charv;
                            TipsAtNode[t][P.up[node]]++;
                        }
                            ...
                            if (inclContrasts) {
                                if (k == 0) {
                                    ...
                                    if (nodeStatus[ordTrt[0]][P.up[node]] < 1) {
                                        if (binCharAssign[ordTrt[0]][P.up[node]] == 0) {
                                            binCharAssign[ordTrt[0]][P.up[node]] = 1;
                                            binChar[ordTrt[0]][P.up[node]]       = bin;
                                            nodeStatus[ordTrt[0]][P.up[node]]    = -1;
                                        }
                                        ...
                                        // contrast test.
                                        else if (binChar[ordTrt[0]][node] != binChar[ordTrt[0]][P.up[node]]) {
                                            nodeStatus[ordTrt[0]][P.up[node]] = 1;
                                            if (nodeStatusCons[ordTrt[0]][P.up[node]] != 0) nodeStatusCons[ordTrt[0]][P.up[node]] = 1;
                                        }
                                    }
                ...
                if ((P.depth[node] == depth) && (P.noat[node] != 0)) {
                    ...
                    for (k = 0; k <= T.ntraits - 1; k++) {
                        ...
                        meanTipChar[t][node] = sumTipChar[t][node] / (float) TipsAtNode[t][node];
                        sdTipChar[t][node] =
                            (sumsqTipChar[t][node] - ((sumTipChar[t][node] * sumTipChar[t][node]) / (float) TipsAtNode[t][node])) /
                            (float) (TipsAtNode[t][node] - 1);
                        if (sdTipChar[t][node] < 0) (sdTipChar[t][node] = 0);
                        ssTipChar[t][node]      = sdTipChar[t][node] * (TipsAtNode[t][node] - 1);
                        ssAmongTips[t][node]    = ssTipChar[t][node] - ssWithinTips[t][node];
                        percVarDivTips[t][node] = ssAmongTips[t][node] / ssTipChar[t][node];
                        sdTipChar[t][node]      = (float) (sqrt((double) sdTipChar[t][node]));

                        // calculate node values
                        if ((Debug) && (run == 0)) {
                            printf(
                                "%d\t%f\t%f\t%f\t%d\n", node, sumBLNChar[t][node], BLwts[node], sumNodeChar[t][node], addedAtNode[t][node]
                            );
                        }
                        if (NoBL == 0)
                            nodeChar[t][node] = sumBLNChar[t][node] / BLwts[node];
                        else
                            nodeChar[t][node] = sumNodeChar[t][node] / addedAtNode[t][node];
                        tmpf                 = nodeChar[t][node] - meanTipChar[t][node];
                        ssTipsVNode[t][node] = ssTipChar[t][node] + TipsAtNode[t][node] * tmpf * tmpf;
                        wtdNChar[t][node]    = sumWtdNChar[t][node] / TipsAtNode[t][node];
                        tmpf                 = wtdNChar[t][node] - nodeChar[t][node];
                        ssAmongNodes[t][node] =
                            (sumsqWtdNChar[t][node] - ((sumWtdNChar[t][node] * sumWtdNChar[t][node]) / TipsAtNode[t][node]));
                        ssAmongNodes[t][node]    = ssAmongNodes[t][node] + TipsAtNode[t][node] * tmpf * tmpf;
                        percVarDivNodes[t][node] = ssAmongNodes[t][node] / (ssAmongNodes[t][node] + ssWithinNodes[t][node]);
                        sdNodeChar[t][node] =
                            (sumsqNodeChar[t][node] - ((sumNodeChar[t][node] * sumNodeChar[t][node]) / addedAtNode[t][node]));
                        if (sdNodeChar[t][node] < 0) (sdNodeChar[t][node] = 0);
                        tmpf                = nodeChar[t][node] - sumNodeChar[t][node] / addedAtNode[t][node];
                        sdNodeChar[t][node] = sdNodeChar[t][node] + addedAtNode[t][node] * tmpf * tmpf;
                        sdNodeChar[t][node] = sqrt(sdNodeChar[t][node] / (addedAtNode[t][node] - 1));
                        ...
                        sumsqNodeSd[t] += sdNodeChar[t][node] * sdNodeChar[t][node];
                        ...
                        if (node != 0) {
                            addedAtNode[t][P.up[node]]++;
                            if ((run == 0) && (k == 0)) Tbl[node] = P.bl[node] + 1 / BLwts[node];
                            sumBLNChar[t][P.up[node]] += nodeChar[t][node] / Tbl[node];
                            if ((run == 0) && (k == 0)) BLwts[P.up[node]] += 1 / Tbl[node];
                            sumNodeChar[t][P.up[node]]   += nodeChar[t][node];
                            sumsqNodeChar[t][P.up[node]] += (nodeChar[t][node] * nodeChar[t][node]);
                            sumWtdNChar[t][P.up[node]]   += nodeChar[t][node] * TipsAtNode[t][node];
                            sumsqWtdNChar[t][P.up[node]] += (nodeChar[t][node] * nodeChar[t][node] * TipsAtNode[t][node]);
                            ssWithinNodes[t][P.up[node]] += ssTipsVNode[t][node];

                            sumTipChar[t][P.up[node]]    += sumTipChar[t][node];
                            sumsqTipChar[t][P.up[node]]  += sumsqTipChar[t][node];
                            TipsAtNode[t][P.up[node]]    += TipsAtNode[t][node];
                            ssWithinTips[t][P.up[node]]  += ssTipChar[t][node];
                            sumsqTipCharG[t][P.up[node]] += sumsqTipCharG[t][node];
                            ssBG1[t][P.up[node]]         += (sumTipChar[t][node] * sumTipChar[t][node]) / TipsAtNode[t][node];
                        }

                        if (T.type[ordTrt[0]] > 1) nodeStatus[ordTrt[0]][node] = 1;
                        ...
                        if (inclContrasts) {
                            if (T.type[ordTrt[0]] == 0) {
                                ...
                                //up were the same.
                                meanNodeCharBin[t][node] = meanNodeCharBin[t][node] / addedAtNodeBin[t][node];
                                //If all binary vals passed up are the same
                                if ((nodeStatus[ordTrt[0]][node] == -1) && (node != 0)) // check for root
                                {
                                    meanNodeCharBin[t][P.up[node]] += meanNodeCharBin[t][node];
                                    addedAtNodeBin[t][P.up[node]]++;
                                }
                                ...
                                if (k == 0) {
                                    ...
                                    if (nodeStatusCons[ordTrt[0]][node] >= 0) nodeStatusCons[ordTrt[0]][P.up[node]] = 0;
                                    //If all binaries the same so far, then pass
                                    //binary values up
                                    if (nodeStatus[ordTrt[0]][node] == -1) {
                                        if (binCharAssign[ordTrt[0]][P.up[node]] == 0) {
                                            binCharAssign[ordTrt[0]][P.up[node]] = 1;
                                            binChar[ordTrt[0]][P.up[node]]       = binChar[ordTrt[0]][node];
                                            nodeStatus[ordTrt[0]][P.up[node]]    = -1;
                                        }
                                        ...
                                        //contrast flag = 1.
                                        else if (binChar[ordTrt[0]][P.up[node]] != binChar[ordTrt[0]][node]) {
                                            nodeStatus[ordTrt[0]][P.up[node]] = 1;
                                            if (nodeStatusCons[ordTrt[0]][P.up[node]] != 0) nodeStatusCons[ordTrt[0]][P.up[node]] = 1;
                                        }
                                    }
                                    ...
                                    //to look up, then set contrast flag = 0
                                        ...
                                        if ((nodeStatus[ordTrt[0]][P.up[node]] == -1) && (binCharAssign[ordTrt[0]][P.up[node]] == 0))
                                            nodeStatus[ordTrt[0]][P.up[node]] = 0;
                                    }
                                }
                        ...
                        sumsqSDTree[t] += sdNodeChar[t][node] * sdNodeChar[t][node];
                        internalNodes[t]++;
                        ...
                        if (nodeChar[t][node] <= MeanNodeChar[t][node]) MeanSigL[t][node]++;
                        if (sdNodeChar[t][node] <= SdNodeChar[t][node]) SdSigL[t][node]++;
                        if (meanTipChar[t][node] <= MeanTipChar[t][node]) TipMeanSigL[t][node]++;
                        if (sdTipChar[t][node] <= SdTipChar[t][node]) TipSdSigL[t][node]++;
                        if (nodeChar[t][node] >= MeanNodeChar[t][node]) MeanSigH[t][node]++;
                        if (sdNodeChar[t][node] >= SdNodeChar[t][node]) SdSigH[t][node]++;
                        if (meanTipChar[t][node] >= MeanTipChar[t][node]) TipMeanSigH[t][node]++;
                        if (sdTipChar[t][node] >= SdTipChar[t][node]) TipSdSigH[t][node]++;
                    }
                }
            }
        }
        ...
        for (k = 0; k <= T.ntraits - 1; k++) {
            ...
            sd_SDTree[t] =
                sqrt((sumsqSDTree[t] - ((sumSDTree[t] * sumSDTree[t]) / (float) internalNodes[t])) / (float) (internalNodes[t] - 1));
            //Depth correlation calculation
            ss1          = sumsqDepth[t] - (sumDepth[t] * sumDepth[t]) / internalNodes[t];
            ss2          = sumsqNodeSd[t] - (sumNodeSd[t] * sumNodeSd[t]) / internalNodes[t];
            sp           = sumprDpNvr[t] - (sumDepth[t] * sumNodeSd[t]) / internalNodes[t];
            ...
            if (meanSDTree[t] <= MeanSDTree[t]) SDTreeSigL[t]++;
            if (sd_SDTree[t] <= SD_SDTree[t]) SD_SDTreeSigL[t]++;
            if (depthCorr[t] <= DepthCorr[t]) DepthCorrSigL[t]++;
            if (meanSDTree[t] >= MeanSDTree[t]) SDTreeSigH[t]++;
            if (sd_SDTree[t] >= SD_SDTree[t]) SD_SDTreeSigH[t]++;
            if (depthCorr[t] >= DepthCorr[t]) DepthCorrSigH[t]++;
            //see results from each randomization
            if (0)
                if (k == 0) printf("%d\t%f\t%f\t%f\t%f\n", run, meanTipChar[t][0], meanSDTree[t], sd_SDTree[t], depthCorr[t]);
        }
        ...
        if (inclContrasts) {
            for (node = 0; node < P.nnodes; node++) {
                ...
                {
                    if ((Debug) && (run == 0)) printf("entering contrast loop %d\t%d\n", node, P.noat[node]);
                    ...
                    for (i = 0; i <= P.noat[node] - 1; i++) // Initialize
                    {
                        ...
                        for (t = 0; t <= T.ntraits - 1; t++) dChar[t][i] = 0.0;
                    }
                    ...
                    do {
                        swap = 0;
                        for (i = 0; i < dtx - 1; i++) {
                            if (dChar[ordTrt[0]][ordChar1[i]] > dChar[ordTrt[0]][ordChar1[i + 1]]) {
                                tmp             = ordChar1[i];
                    ...
                    if (T.type[ordTrt[0]] == 0) {
                        ...
                        for (i = 0; i < dtx; i++)
                            if ((int) dChar[ordTrt[0]][ordChar1[i]] == bin)
                                ...
                                dtxhi++;
                        if ((run == 0) && (dtx != dtxlow + dtxhi)) printf("dtx sum error");
                        ...
                        if (dtx % 2 == 0) {
                            ...
                            tmpsum = 0.0;
                            for (i = 0; i < dtx; i++) tmpsum += dChar[ordTrt[0]][ordChar1[i]];
                            tmpsum = tmpsum / dtx;
                    ...
                    for (k = 0; k <= T.ntraits - 1; k++) {
                        ...
                        dWtHi  = 0.0;
                        if (T.type[ordTrt[0]] == 0) {
                            for (i = 0; i < dtxlow; i++) dCharLow[t] += dChar[t][ordChar1[i]];
                            for (i = dtxlow; i <= dtx - 1; i++) dCharHi[t] += dChar[t][ordChar1[i]];
                            ...
                            if (NoBL == 0) {
                                for (i = 0; i < dtxlow; i++) {
                                    dCharLow[t] += dChar[t][ordChar1[i]] / dBL[t][ordChar1[i]];
                                    dWtLow      += 1 / dBL[t][ordChar1[i]];
                                }
                                for (i = dtxlow; i <= dtx - 1; i++) {
                                    dCharHi[t] += dChar[t][ordChar1[i]] / dBL[t][ordChar1[i]];
                            ...
                            if (NoBL == 0) {
                                ...
                                contrast[t]      = (dCharHi[t] - dCharLow[t]) / sdContrast[node];
                            } else {
                                sdContrast[node] = 0.0;
                                contrast[t]      = (dCharHi[t] - dCharLow[t]);
                                if (DOTuseSD == 0) {
                                    wtdContAge[t] += fabs(contrast[t]) * P.age[node];
                                    ageWt[t]      += fabs(contrast[t]);
                                }
                            }
                            if ((Debug) && (run == 0))
                                printf("CONT:\t%d\t%d\t%f\t%f\t%f\t%f\t%f\n", node, t, dCharLow[t], dCharHi[t], contrast[t], dWtLow, dWtHi);
                        }
                        ...
                        if (run == 0) {
                            ...
                            if (minAContrast[t] == 0) minAContrast[t] = fabs(contrast[t]);
                            if (fabs(contrast[t]) < minAContrast[t]) minAContrast[t] = fabs(contrast[t]);
                            if (fabs(contrast[t]) > maxAContrast[t]) maxAContrast[t] = fabs(contrast[t]);
                        }
                        ...
                        if (T.type[ordTrt[0]] == 0) // trait 1 is binary
                        {
                            if (nodeStatus[ordTrt[0]][node] == 1) {
                                ...
                                if (k == 1) nSignCont++;
                                if (contrast[t] > 0) signCont[t] += 1;
                                if (contrast[t] == 0) signCont[t] += 0.5;
                                sumCon[t]   += contrast[t];
                            ...
                            if (nodeStatusCons[ordTrt[0]][node] == 1) {
                                //if ((run == 0)&&(t == 1)) printf("enter binary contrast calc\n");
                                if (k == 1) nSignContCons++;
                                if (contrast[t] > 0) signContCons[t] += 1;
                                if (contrast[t] == 0) signContCons[t] += 0.5;
                        ...
                        // trait 1 is ordered multistate or continuous
                            ...
                            if (k != 0) {
                                if (k == 1) nContrast++;
                                sumprodCon[t] += contrast[ordTrt[0]] * contrast[t];
                                if (k == 1) nSignCont++;
                                if (contrast[t] > 0) signCont[t] += 1;
                                if (contrast[t] == 0) signCont[t] += 0.5;
                            }
                    ...
                    if ((Debug) && (run == 0)) {
                        printf("%d\t%d\t%f\t%f\n", nodeStatus[ordTrt[0]][node], nodeStatusCons[ordTrt[0]][node], dCharLow[0], dCharLow[1]);
                        printf("%d\t%d\t%f\t%f\n", nodeStatus[ordTrt[0]][node], nodeStatusCons[ordTrt[0]][node], dCharHi[0], dCharHi[1]);
                        printf("%d\t%d\t%f\t%f\n", nodeStatus[ordTrt[0]][node], nodeStatusCons[ordTrt[0]][node], contrast[0], contrast[1]);
                    }

                    if ((Debug) && (run == 0)) printf("\n");
                }
            }
            ...
            for (k = 0; k <= T.ntraits - 1; k++) {
                ...
                if (run == 0) wtdContAgeObs[t] = wtdContAge[t];
            }
                ...
                if (P.noat[node] != 0) // Internal nodes only
                {
                    for (k = 0; k <= T.ntraits - 1; k++) {
                        ...
                        if (AbsContrast[t][node] >= midAContrast[t]) {
                            ...
                            if (run == -1) printf("t,node,q,age %d\t%d\t%f\n", t, node, P.age[node]);
                        }
                        if (k > 0) quadrant[t][q[0] + q[t]]++;
                    }
                }
            }

            //Calculate SvS and DOT
            aveAgeLargeContrasts[ordTrt[0]] = aveAgeLargeContrasts[ordTrt[0]] / nLargeContrasts[ordTrt[0]];
            for (k = 0; k <= T.ntraits - 1; k++) {
                t                       = ordTrt[k];
                SvS[t]                  = (float) quadrant[t][1] / (float) (quadrant[t][1] + quadrant[t][2]);
                aveAgeLargeContrasts[t] = aveAgeLargeContrasts[t] / nLargeContrasts[t];
                ...
                if (LowSig) {
                    if (SvS[t] <= SvSObs[t]) SvSSig[t]++;
                    if (DOT[t] <= DOTObs[t]) DOTSig[t]++;
                } else {
                    if (SvS[t] >= SvSObs[t]) SvSSig[t]++;
                    if (DOT[t] >= DOTObs[t]) DOTSig[t]++;
                }
            ...
            for (k = 1; k <= T.ntraits - 1; k++) {
                ...
                if (signCont[t] <= SignCont[t]) SignContSigL[t]++;
                if (signContCons[t] <= SignContCons[t]) SignContConsSigL[t]++;
                if (signCont[t] >= SignCont[t]) SignContSigH[t]++;
                if (signContCons[t] >= SignContCons[t]) SignContConsSigH[t]++;
            }
            ...
            if (T.type[ordTrt[0]] == 0) {
                for (k = 1; k <= T.ntraits - 1; k++) {
                    t             = ordTrt[k];
                    sdCon[t]      = sumsqCon[t] - ((sumCon[t] * sumCon[t]) / nSignCont);
                    sdCon[t]      = sqrt(sdCon[t] / (nSignCont - 1));
                    sdConCons[t]  = sumsqConCons[t] - ((sumConCons[t] * sumConCons[t]) / nSignContCons);
                    ...
                    if (sumCon[t] <= SumCon[t]) PICcorrSigL[t]++;
                    if (sumConCons[t] <= SumConCons[t]) PICcorrSignConsL[t]++;
                    if (sumCon[t] >= SumCon[t]) PICcorrSigH[t]++;
                    if (sumConCons[t] >= SumConCons[t]) PICcorrSignConsH[t]++;
                    //printf("%f\n", PICcorr[t]);
                }
            } else {
                for (k = 1; k <= T.ntraits - 1; k++) {
                    t           = ordTrt[k];
                    pic_corr[t] = sumprodCon[t] / sqrt(sumsqCon[ordTrt[0]] * sumsqCon[t]);
                    if (run == 0) PICcorr[t] = pic_corr[t];
                    if (pic_corr[t] <= PICcorr[t]) PICcorrSigL[t]++;
                    if (pic_corr[t] >= PICcorr[t]) PICcorrSigH[t]++;
                    //printf("%f\n", PICcorr[t]);
                }
            }
        }
        ...
        for (k = 0; k < T.ntraits; k++) {
            ...
            varC = 0.0;
            for (i = 0; i < count; i++) varC += pow(c[i], 2);
            varC = varC / (float) (count - 1);
            if (run == 0) VarC[k] = varC;
            if (varC <= VarC[k]) varCRankLow[k]++;
            if (varC >= VarC[k]) varCRankHigh[k]++;
        }
    }
    ...
    if (Output == 0) {
            ...
            for (k = 0; k <= T.ntraits - 1; k++) {
                ...
                for (node = 0; node < P.nnodes; node++) {
                    if (P.noat[node] != 0) {
                        ...
                        printf("%4d %6.2f ", TipsAtNode[t][node], MeanTipChar[t][node]);
                        printf("%4d %6.3f %4d ", TipMeanSigL[t][node], SdTipChar[t][node], TipSdSigL[t][node]);
                        printf("%6.2f %4d ", MeanNodeChar[t][node], MeanSigL[t][node]);
                        printf("%6.3f %4d ", SdNodeChar[t][node], SdSigL[t][node]);
                        printf("%s\n", P.taxon[node]);
                    }
                }
            }
        ...
        for (k = 0; k <= T.ntraits - 1; k++) {
            t = ordTrt[k];
            printf("%s\t%d\t%.3f\t%d\t%d\n", T.trname[t], TipsAtNode[t][0], VarC[t], varCRankLow[t], varCRankHigh[t]);
        }
        if (inclContrasts) {
            ...
            for (k = 1; k <= T.ntraits - 1; k++) {
                ...
                if (T.type[t] != 0) {
                    if (T.type[ordTrt[0]] == 0) {
                        ...
                        printf("%2d %6.3f %5.1f %5d\n", t + 1, PICcorr[t], SignCont[t], nContrast);
                    }
                }
            }
        }
    ...
    if (Output == 1) {
            ...
            for (k = 0; k <= T.ntraits - 1; k++) {
                ...
                for (node = 0; node < P.nnodes; node++) {
                    if (P.noat[node] != 0) {
                        ...
                        printf("\t%f", P.age[node]);
                        printf("\t%d\t%d\t%.3f\t", TipsAtNode[t][node], P.noat[node], MeanTipChar[t][node]);
                        printf(
                            "%d\t%d\t%.4f\t%d\t%d\t",
                            TipMeanSigL[t][node],
                            TipMeanSigH[t][node],
                        ...
                        printf("%.3f\t%d\t%d\t", MeanNodeChar[t][node], MeanSigL[t][node], MeanSigH[t][node]);
                        printf("%.3f\t%d\t%d\t", SdNodeChar[t][node], SdSigL[t][node], SdSigH[t][node]);
                        if (Debug) printf("%f\t%f\t%f\t", BLwts[node], P.bl[node], Tbl[node]);
                        printf("%f\t%f\t", SSTipChar[t][0], SSTipChar[t][node]);
                        tmpf1 = SSAmongNodes[t][node] / (SSAmongNodes[t][node] + SSWithinNodes[t][node]);
                        tmpf2 = SSTipsVNode[t][node] / SSTipsVNode[t][0];
                        printf("%f\t", tmpf1);
                        printf("%f\t", tmpf2);
                        printf("%f\t", tmpf1 * tmpf2);
                        printf("%f\t%f\t%f\t%f", SSTipsVNode[t][0], SSTipsVNode[t][node], SSAmongNodes[t][node], SSWithinNodes[t][node]);
                        if (Debug) printf("\t%d", P.depth[node]);
                        printf("\n");
                    }
                }
            }
        ...
        if ((aotOut == 0) || (aotOut == 2)) {
                ...
                if (aotOut == 0) printf("\nOutput of independent contrast values by node\n");
            {
                ...
                for (i = 0; i <= T.ntraits - 1; i++) printf("\tContrast%d", i + 1);
                if (T.type[ordTrt[0]] == 0)
                    printf("\tNodeType");
                else
                    printf("\tContrastSD");
                for (i = 0; i <= T.ntraits - 1; i++) printf("\tLowVal%d\tHiVal%d", i + 1, i + 1);
                printf("\n");
                ...
                for (node = 0; node < P.nnodes; node++) {
                    if (nodeStatus[ordTrt[0]][node] == 1) {
                        ...
                        for (i = 0; i <= T.ntraits - 1; i++) printf("\t%f", Contrast[ordTrt[i]][node]);
                        if (T.type[ordTrt[0]] == 0)
                            if (nodeStatusCons[ordTrt[0]][node] == 1)
                                printf("\tST");
                            else
                        ...
                        for (i = 0; i <= T.ntraits - 1; i++) printf("\t%f\t%f", DCharLow[ordTrt[i]][node], DCharHi[ordTrt[i]][node]);
                        printf("\n");
                    }
                }
            }
        ...
        if (aotOut == 0) {
                ...
                for (node = 0; node < P.nnodes; node++) {
                    if (P.noat[node] == 0) {
                        for (node = 0; node < P.nnodes; node++)
                            for (k = 0; k <= T.ntraits - 1; k++) {
                                t = ordTrt[k];
                                printf("%d\t%s\t%d\t%d\t%f\n", node, P.taxon[node], P.depth[node], t + 1, MeanNodeChar[t][node]);
                            }
                    }
                }
            }
            ...
            if (showDOT) {
                ...
                for (k = 0; k <= T.ntraits - 1; k++) {
                    t = ordTrt[k];
                    printf("%s\t%f\t%f\t%d\n", T.trname[t], wtdContAgeObs[t], DOTObs[t], DOTSig[t]);
                }
                printf("\n");
                // printf("%f\t%d\n",SvSObs[1],SvSSig[1]);
                // printf("%f\t%d\n",DOTObs[1],DOTSig[1]);
    ...
    if (Output == 2) {
        ...
        for (k = 0; k < T.ntraits; k++) {
            ...
            for (node = 1; node < P.nnodes; node++) {
                ...
                if (Out[t].arenotes == 1) strcpy(Out[t].notes[node], "");
...
void RandArrayT(int** RndArr, int nTaxa, int run, int nTraits) {
    ...
    for (t = 0; t < nTraits; t++) {
        if (run == 0) {
            for (i = 0; i < nTaxa; i++) RndArr[i][t] = i;
        } else {
            for (i = 0; i < nTaxa; i++) data[i] = i;
            for (loop = 0; loop < nTaxa; loop++) {
                size = nTaxa - loop;
                // pick = ((int) (((float) size * rand()) / (RAND_MAX+1.0)));
                pick = (int) ((((double) rand()) / ((double) RAND_MAX)) * (double) size);
                if (pick == size) pick = pick - 1;
                if (pick > size) printf("RndArr out of bounds, run = %d, loop = %d", run, loop);
                RndArr[loop][t] = data[pick];
                for (j = pick; j < size - 1; j++) data[j] = data[j + 1];
                //printf("%d\t%d\n", t, RndArr[loop][t]);
