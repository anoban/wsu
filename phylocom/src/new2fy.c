...
phylo New2fy(char filename[50]) {
    ...
    for (p = 0; p < Xtree.nnodes; p++) {
        Xtree.noat[p]  = 0;
        Xtree.depth[p] = 0;
        for (q = 0; q <= MAXTAXONLENGTH; q++) Xtree.taxon[p][q] = 0;
    }
    ...
    while (i < lng) {
        ...
        else if ((((line[i] >= 65) && (line[i] <= 90)) || ((line[i] >= 97) && (line[i] <= 122)) || (line[i] == 45) || (line[i] == 95)) &&
                 (strncmp(&line[i - 1], ")", 1) == 0)) {
            // clear iname
            for (q = 0; q < MAXTAXONLENGTH; q++) iname[q] = 0;

            while ((strncmp(&line[i], ":", 1) != 0) && (strncmp(&line[i], ",", 1) != 0) && (strncmp(&line[i], ")", 1) != 0) &&
                   (strncmp(&line[i], "[", 1) != 0) && (strncmp(&line[i], ";", 1) != 0) && (strncmp(&line[i], "]", 1) != 0)) {
    ...
    // tidy up
    for (i = 0; i <= nodei; i++)
        if (branchlengths == 0) Xtree.bl[i] = 1.0;
    ...
    for (i = 0; i < Xtree.nnodes; i++) {
        ...
        if (strlen(Xtree.taxon[i]) > 0) Xtree.ntaxa++;
