...
def get_argparser():
    ...
    available_models = sorted(
        ...
        if name.islower() and not (name.startswith("__") or name.startswith("_")) and callable(network.modeling.__dict__[name])
    )
    ...
    parser.add_argument(
        "--loss_type", type=str, default="cross_entropy", choices=["cross_entropy", "focal_loss"], help="loss type (default: False)"
    )
    ...
    # PASCAL VOC Options
    parser.add_argument(
        "--year", type=str, default="2012", choices=["2012_aug", "2012", "2011", "2009", "2008", "2007"], help="year of VOC"
    )
...
def get_dataset(opts):
    ...
    if opts.dataset == "voc":
        ...
        if opts.crop_val:
            ...
            val_transform = et.ExtCompose([et.ExtToTensor(), et.ExtNormalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])])
        train_dst = VOCSegmentation(
            root=opts.data_root, year=opts.year, image_set="train", download=opts.download, transform=train_transform
        )
...
def main():
    ...
    optimizer = torch.optim.SGD(
        params=[{"params": model.backbone.parameters(), "lr": 0.1 * opts.lr}, {"params": model.classifier.parameters(), "lr": opts.lr}],
        lr=opts.lr,
        momentum=0.9,
