...
import cv2
import numpy as np
from checks import check_file, check_requirements
from downloads import download, zip_directory
            ...
            for ann in anns:
                ...
                if box not in bboxes:
                    ...
                    if use_keypoints and ann.get("keypoints") is not None:
                        keypoints.append(box + (np.array(ann["keypoints"]).reshape(-1, 3) / np.array([w, h, 1])).reshape(-1).tolist())
...
def create_synthetic_coco_dataset():
    ...
    def create_synthetic_image(image_file: Path):
        ...
        if not image_file.exists():
            ...
            Image.new("RGB", size=size, color=(random.randint(0, 255), random.randint(0, 255), random.randint(0, 255))).save(image_file)
...
async def convert_ndjson_to_yolo(ndjson_path: str | Path, output_path: str | Path | None = None) -> Path:
    ...
    async with aiohttp.ClientSession() as session:
        pbar = TQDM(total=len(image_records), desc=f"Converting {ndjson_path.name} → {dataset_dir} ({len(image_records)} images)")
