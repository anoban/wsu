...
from utils import LOGGER, RANK, nms, ops
...
class DetectionValidator(BaseValidator):
    ...
    def _prepare_batch(self, si: int, batch: dict[str, Any]) -> dict[str, Any]:
        ...
        return {"cls": cls, "bboxes": bbox, "ori_shape": ori_shape, "imgsz": imgsz, "ratio_pad": ratio_pad, "im_file": batch["im_file"][si]}
    ...
    def update_metrics(self, preds: list[dict[str, torch.Tensor]], batch: dict[str, Any]) -> None:
        ...
        for si, pred in enumerate(preds):
            ...
            if self.args.save_txt:
                self.save_one_txt(
                    predn_scaled, self.args.save_conf, pbatch["ori_shape"], self.save_dir / "labels" / f"{Path(pbatch['im_file']).stem}.txt"
                )
    ...
    def print_results(self) -> None:
        ...
        if self.args.verbose and not self.training and self.nc > 1 and len(self.metrics.stats):
            for i, c in enumerate(self.metrics.ap_class_index):
                LOGGER.info(pf % (self.names[c], self.metrics.nt_per_image[c], self.metrics.nt_per_class[c], *self.metrics.class_result(i)))
    ...
    def get_dataloader(self, dataset_path: str, batch_size: int) -> torch.utils.data.DataLoader:
        ...
        return build_dataloader(
            dataset, batch_size, self.args.workers, shuffle=False, rank=-1, drop_last=self.args.compile, pin_memory=self.training
        )
    ...
    def plot_val_samples(self, batch: dict[str, Any], ni: int) -> None:
        ...
        """
        plot_images(
            labels=batch, paths=batch["im_file"], fname=self.save_dir / f"val_batch{ni}_labels.jpg", names=self.names, on_plot=self.on_plot
        )
    ...
    def scale_preds(self, predn: dict[str, torch.Tensor], pbatch: dict[str, Any]) -> dict[str, torch.Tensor]:
        ...
        return {
            **predn,
            "bboxes": ops.scale_boxes(pbatch["imgsz"], predn["bboxes"].clone(), pbatch["ori_shape"], ratio_pad=pbatch["ratio_pad"]),
        }
    ...
    def coco_evaluate(
        self, stats: dict[str, Any], pred_json: str, anno_json: str, iou_types: str | list[str] = "bbox", suffix: str | list[str] = "Box"
    ) -> dict[str, Any]:
