...
class Mosaic(BaseMixTransform):
    ...
    def _mosaic4(self, labels: dict[str, Any]) -> dict[str, Any]:
        ...
        Examples:
            >>> mosaic = Mosaic(dataset, imgsz=640, p=1.0, n=4)
            >>> labels = {"img": np.random.rand(480, 640, 3), "mix_labels": [{"img": np.random.rand(480, 640, 3)} for _ in range(3)]}
...
class RandomPerspective:
    ...
    @staticmethod
    def box_candidates(
        box1: np.ndarray, box2: np.ndarray, wh_thr: int = 2, ar_thr: int = 100, area_thr: float = 0.1, eps: float = 1e-16
    ) -> np.ndarray:
