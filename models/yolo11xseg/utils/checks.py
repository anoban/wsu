...
@functools.lru_cache
def check_version(
    current: str = "0.0.0", required: str = "0.0.0", name: str = "version", hard: bool = False, verbose: bool = False, msg: str = ""
) -> bool:
    ...
    if ONLINE and IS_PIP_PACKAGE:
        try:
            ...
            if check_version(__version__, f"<{latest}"):  # check if current version is < latest version
                LOGGER.info(f"New https://pypi.org/project/ultralytics/{latest} available 😃 Update with 'pip install -U ultralytics'")
                return True
        except Exception:
            pass
    return False
            ...
            try:
                ...
                if e.output and "No virtual environment found" in e.output:
                    return subprocess.check_output(
                        base.replace("uv pip install", "uv pip install --system"), shell=True, stderr=subprocess.STDOUT, text=True
                    )
                raise
        return subprocess.check_output(f"pip install --no-cache-dir {packages} {commands}", shell=True, stderr=subprocess.STDOUT, text=True)
