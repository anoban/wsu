...
class BaseDataProvider:
    def __init__(self, cfg: BaseDataProviderConfig) -> None:
        self.cfg = cfg
        ...
        self.train = self.build_dataloader(self.train_dataset, batch_size=self.cfg.batch_size, shuffle=True, drop_last=cfg.train_drop_last)
        self.valid = self.build_dataloader(self.val_dataset, batch_size=self.cfg.batch_size, shuffle=False, drop_last=False)
        self.test = self.build_dataloader(self.test_dataset, batch_size=self.cfg.batch_size, shuffle=False, drop_last=False)
    ...
    def build_dataloader(self, dataset: Optional[Dataset], batch_size: int, shuffle: bool, drop_last: bool) -> Optional[DataLoader]:
        if dataset is None:
            return None
        sampler = DistributedRangedSampler(dataset, self.num_replicas, self.rank, shuffle=shuffle, seed=self.cfg.seed, drop_last=drop_last)
        return DataLoader(
            dataset=dataset, batch_size=batch_size, sampler=sampler, num_workers=self.cfg.n_worker, pin_memory=True, drop_last=drop_last
        )
