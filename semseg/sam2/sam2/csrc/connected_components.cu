...
namespace cc2d {
    ...
    template<typename T> __device__ __forceinline__ unsigned char hasBit(T bitmap, unsigned char pos) { return (bitmap >> pos) & 1; }
    ...
    __global__ void merge(uint8_t* img, int32_t* label, const uint32_t W, const uint32_t H) {
        ...
        if (P > 0) {
            ...
            // top-left pixel
            if (hasBit(P, 0) && img[idx - W - 1]) union_(label, idx, idx - 2 * W - 2); // top left block

            if ((hasBit(P, 1) && img[idx - W]) || (hasBit(P, 2) && img[idx - W + 1])) union_(label, idx, idx - 2 * W); // top bottom block

            if (hasBit(P, 3) && img[idx + 2 - W]) union_(label, idx, idx - 2 * W + 2); // top right block

            if ((hasBit(P, 4) && img[idx - 1]) || (hasBit(P, 8) && img[idx + W - 1])) union_(label, idx, idx - 2); // just left block
        }
    }
    ...
    __global__ void final_counting(
        const int32_t* label, const int32_t* count_init, int32_t* count_final, const int32_t W, const int32_t H
    ) {
        const uint32_t row = (blockIdx.y * blockDim.y + threadIdx.y);
        const uint32_t col = (blockIdx.x * blockDim.x + threadIdx.x);
        const uint32_t idx = row * W + col;
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) { m.def("get_connected_componnets", &get_connected_componnets, "get_connected_componnets"); }
