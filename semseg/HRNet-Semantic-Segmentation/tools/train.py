...
def main():
    ...
    testloader = torch.utils.data.DataLoader(
        test_dataset, batch_size=batch_size, shuffle=False, num_workers=config.WORKERS, pin_memory=True, sampler=test_sampler
    )
    ...
    if config.TRAIN.OPTIMIZER == "sgd":
        ...
        optimizer = torch.optim.SGD(
            params, lr=config.TRAIN.LR, momentum=config.TRAIN.MOMENTUM, weight_decay=config.TRAIN.WD, nesterov=config.TRAIN.NESTEROV
        )
    else:
    ...
    for epoch in range(last_epoch, end_epoch):
        ...
        if epoch >= config.TRAIN.END_EPOCH:
            ...
            train(
                config, epoch, config.TRAIN.END_EPOCH, epoch_iters, config.TRAIN.LR, num_iters, trainloader, optimizer, model, writer_dict
            )
        ...
        if args.local_rank <= 0:
            ...
            torch.save(
                {"epoch": epoch + 1, "best_mIoU": best_mIoU, "state_dict": model.module.state_dict(), "optimizer": optimizer.state_dict()},
