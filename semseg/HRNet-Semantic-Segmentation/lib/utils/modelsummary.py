...
def get_model_summary(model, *input_tensors, item_length=26, verbose=False):
    ...
    def add_hooks(module):
        ...
        def hook(module, input, output):
            ...
            if class_name.find("Conv") != -1 and hasattr(module, "weight"):
                flops = (
                    torch.prod(torch.LongTensor(list(module.weight.data.size()))) * torch.prod(torch.LongTensor(list(output.size())[2:]))
                ).item()
    ...
    details += (
        "Total Multiply Adds (For Convolution and Linear Layers only): {:,} GFLOPs".format(flops_sum / (1024**3))
        + os.linesep
        + "-" * space_len * 5
        + os.linesep
    )
