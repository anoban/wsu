    ...
    for i_iter, batch in enumerate(trainloader, 0):
        ...
        if i_iter % config.PRINT_FREQ == 0 and dist.get_rank() == 0:
            msg = "Epoch: [{}/{}] Iter:[{}/{}], Time: {:.2f}, lr: {}, Loss: {:.6f}".format(
                epoch, num_epoch, i_iter, epoch_iters, batch_time.average(), [x["lr"] for x in optimizer.param_groups], ave_loss.average()
            )
            logging.info(msg)
    ...
    with torch.no_grad():
        for idx, batch in enumerate(testloader):
            ...
            for i, x in enumerate(pred):
                x = F.interpolate(input=x, size=size[-2:], mode="bilinear", align_corners=config.MODEL.ALIGN_CORNERS)

                confusion_matrix[..., i] += get_confusion_matrix(label, x, size, config.DATASET.NUM_CLASSES, config.TRAIN.IGNORE_LABEL)
