...
class COCOStuff(BaseDataset):
    def __init__(
        ...
        super(COCOStuff, self).__init__(ignore_label, base_size, crop_size, downsample_rate, scale_factor, mean, std)
    ...
    def __getitem__(self, index):
        ...
        if "testval" in self.list_path:
            image, border_padding = self.resize_short_length(image, short_length=self.base_size, fit_stride=8, return_padding=True)
        ...
        if "val" in self.list_path:
            image, label = self.resize_short_length(image, label=label, short_length=self.base_size, fit_stride=8)
