...
def display_differences(
    ...
    captions = ["" for m in gt_match] + [
        "{:.2f} / {:.2f}".format(pred_score[i], (overlaps[i, int(pred_match[i])] if pred_match[i] > -1 else overlaps[i].max()))
...
def draw_rois(image, rois, refined_rois, mask, class_ids, class_names, limit=10):
    ...
    for i, id in enumerate(ids):
        ...
        y1, x1, y2, x2 = rois[id]
        p = patches.Rectangle(
            (x1, y1), x2 - x1, y2 - y1, linewidth=2, edgecolor=color if class_id else "gray", facecolor="none", linestyle="dashed"
        )
        ax.add_patch(p)
        # Refined ROI
        if class_id:
...
def plot_overlaps(gt_class_ids, pred_class_ids, pred_scores, overlaps, class_names, threshold=0.5):
    ...
    plt.yticks(
        np.arange(len(pred_class_ids)), ["{} ({:.2f})".format(class_names[int(id)], pred_scores[i]) for i, id in enumerate(pred_class_ids)]
    )
...
def display_weight_stats(model):
    ...
    for l in layers:
        ...
        for i, w in enumerate(weight_values):
            ...
            # Add row
            table.append(
                [weight_name + alert, str(w.shape), "{:+9.4f}".format(w.min()), "{:+10.4f}".format(w.max()), "{:+9.4f}".format(w.std())]
            )
    display_table(table)
