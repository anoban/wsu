...
from sam3.perflib.compile import clone_output_wrapper, compile_wrapper, shape_logging_wrapper
...
from sam3.model.data_misc import BatchedPointer, FindStage, NestedTensor, convert_my_tensors
...
class Sam3MultiplexTracking(Sam3MultiplexBase):
    ...
    def init_state(
        self, resource_path, offload_video_to_cpu=False, async_loading_frames=False, use_torchcodec=False, use_cv2=False, input_is_mp4=False
    ):
    ...
    def _get_processing_order(self, inference_state, start_frame_idx, max_frame_num_to_track, reverse):
        ...
        if all(out is None for out in previous_stages_out) and start_frame_idx is None:
            raise RuntimeError("No prompts are received on any frames. Please add prompt on at least one frame before propagation.")
    ...
    def propagate_in_video(
        ...
        **kwargs,  # To support passing extra args to child classes
        ...
        processing_order, end_frame_idx = self._get_processing_order(
            inference_state, start_frame_idx, max_frame_num_to_track, reverse=reverse
        )
        ...
        for frame_idx in tqdm(processing_order, desc="propagate_in_video", disable=self.rank > 0):
            out = self._run_single_frame_inference(inference_state, frame_idx, reverse, is_instance_processing=is_instance_processing)
                    ...
                    if self.rank == 0:
                        ...
                        for yield_frame_idx, yield_out, removed_obj_ids_snapshot in batch_to_process:
                            ...
                            unconfirmed_status_frame_idx = (
                                yield_frame_idx + unconfirmed_status_delay if not reverse else yield_frame_idx - unconfirmed_status_delay
                            )
                            ...
                            batched_outs.append((yield_out, removed_obj_ids_snapshot, suppressed_obj_ids, unconfirmed_obj_ids))
                        ...
                        if self.postprocess_batch_size > 1:
                            ...
                            postprocessed_outs = []
                            for yield_out, removed_obj_ids_snapshot, suppressed_obj_ids, unconfirmed_obj_ids in batched_outs:
                                postprocessed_out = self._postprocess_output(
                                    inference_state, yield_out, removed_obj_ids_snapshot, suppressed_obj_ids, unconfirmed_obj_ids
                                )
        ...
        if len(postprocess_yield_list) > 0:
            with torch.profiler.record_function("Sam3MultiplexTracking.postprocess_output_batched"):
                if self.rank == 0:
                    ...
                    batched_outs = []
                    frame_indices = []
                    for yield_frame_idx, yield_out, removed_obj_ids_snapshot in postprocess_yield_list:
                        suppressed_obj_ids = yield_out["suppressed_obj_ids"]
                        unconfirmed_status_frame_idx = (
                            yield_frame_idx + unconfirmed_status_delay if not reverse else yield_frame_idx - unconfirmed_status_delay
                        )
        ...
        if self.is_multiplex:
            ...
            if total_num_buckets > 0:
                ...
                logger.info(f"Bucket utilization rate: {bucket_utilization_rate:.2f}%, subscription rate: {subscription_rate:.2f}%")
    ...
    def _run_single_frame_inference(self, inference_state, frame_idx, reverse, is_instance_processing=False):
        ...
        (obj_id_to_mask, obj_id_to_score, tracker_states_local_new, tracker_metadata_new, frame_stats, _) = self._det_track_one_frame(
            frame_idx=frame_idx,
            num_frames=inference_state["num_frames"],
            reverse=reverse,
    ...
    def _postprocess_output(self, inference_state, out, removed_obj_ids=None, suppressed_obj_ids=None, unconfirmed_obj_ids=None):
        ...
        if len(curr_obj_ids) == 0:
            ...
            out_sam2_probs = torch.tensor(
                [(out["obj_id_to_sam2_score"][obj_id] if obj_id in out["obj_id_to_sam2_score"] else 0.0) for obj_id in curr_obj_ids]
            )
        ...
        if out_binary_masks.shape[0] > 1:
            ...
            out_binary_masks = (
                self.tracker._apply_object_wise_non_overlapping_constraints(
                    out_binary_masks.unsqueeze(1), out_sam2_probs.unsqueeze(1).to(out_binary_masks.device), background_value=0
                ).squeeze(1)
            ) > 0
        ...
        if self.running_in_prod:
            with torch.profiler.record_function("Sam3MultiplexTracking._postprocess_output.prod_outputs"):
                out_centers = torch.zeros(out_binary_masks.shape[0], 2, dtype=torch.float32, device=out_binary_masks.device)
    ...
    def _postprocess_output_batched(self, H_video, W_video, batched_outs):
        ...
        for out, removed_obj_ids, suppressed_obj_ids, unconfirmed_obj_ids in batched_outs:
            ...
            frame_data.append((out_obj_ids, out_probs, out_sam2_probs_gpu, out_binary_masks, hide_mask, frame_stats))
        ...
        for idx, frame_i in enumerate(frames_with_objects):
            ...
            if out_masks.shape[0] > 1:
                ...
                out_masks = (
                    self.tracker._apply_object_wise_non_overlapping_constraints(
                        out_masks.unsqueeze(1), out_sam2_probs_cpu.unsqueeze(1).to(out_masks.device), background_value=0
                    ).squeeze(1)
                ) > 0
        ...
        if needs_buffer_init or needs_buffer_resize:
            self.buffer_cpu_batched = {
                "out_obj_ids": torch.zeros(batched_buffer_size, dtype=torch.int64, device="cpu", pin_memory=True),
                "out_probs": torch.zeros(batched_buffer_size, dtype=torch.float32, device="cpu", pin_memory=True),
                "out_boxes_xywh": torch.zeros(batched_buffer_size, 4, dtype=torch.float32, device="cpu", pin_memory=True),
                "out_binary_masks": torch.zeros(batched_buffer_size, H_video, W_video, dtype=bool, device="cpu", pin_memory=True),
            }
            if self.running_in_prod:
                self.buffer_cpu_batched["out_centers"] = torch.zeros(
                    batched_buffer_size, 2, dtype=torch.float32, device="cpu", pin_memory=True
                )
        ...
        for i, data in enumerate(frame_data):
            ...
            if i not in frame_to_offset_count:
                ...
                output_dict = {
                    "out_obj_ids": self.buffer_cpu_batched["out_obj_ids"][buf_offset : buf_offset + num_objects].numpy().copy(),
                    "out_probs": self.buffer_cpu_batched["out_probs"][buf_offset : buf_offset + num_objects].numpy().copy(),
                    "out_boxes_xywh": self.buffer_cpu_batched["out_boxes_xywh"][buf_offset : buf_offset + num_objects].numpy().copy(),
                    "out_binary_masks": self.buffer_cpu_batched["out_binary_masks"][buf_offset : buf_offset + num_objects].numpy().copy(),
                    "frame_stats": frame_stats,
                }
                if all_centers is not None:
    ...
    def _cache_frame_outputs(
        self, inference_state, frame_idx, obj_id_to_mask, suppressed_obj_ids=None, removed_obj_ids=None, unconfirmed_obj_ids=None
    ):
    ...
    def _compile_model(self):
        ...
        self.detector.backbone.language_backbone.encoder.forward = clone_output_wrapper(
            torch.compile(self.detector.backbone.language_backbone.encoder.forward, fullgraph=True, mode="max-autotune")
        )
        ...
        self.tracker.transformer.encoder.forward = shape_logging_wrapper(
            compile_wrapper(self.tracker.transformer.encoder.forward, mode="max-autotune-no-cudagraphs", fullgraph=True, dynamic=True),
        ...
        sam3.model.sam3_video_base._associate_det_trk_compilable = compile_wrapper(
            sam3.model.sam3_video_base._associate_det_trk_compilable, mode="max-autotune-no-cudagraphs", fullgraph=True, dynamic=False
        )

        self.tracker._suppress_object_pw_area_shrinkage = compile_wrapper(
            self.tracker._suppress_object_pw_area_shrinkage, mode="max-autotune-no-cudagraphs", fullgraph=True, dynamic=False
        )
    ...
    def _warm_up_vg_propagation(self, inference_state, start_frame_idx=0):
        ...
        for i in range(num_rounds):
            for num_objects in num_objects_list:
                ...
                if num_objects > 0:
                    inference_state = self.add_fake_objects_to_inference_state(inference_state, num_objects, frame_idx=start_frame_idx)
                ...
                logger.info(f"{i + 1}/{num_rounds} warming up model compilation -- completed round {i + 1} out of {num_rounds}")
        ...
        for _ in tqdm(range(num_iters)):
            for b in range(1, self.num_obj_for_compile + 1):
                for i in range(1, self.tracker.max_cond_frames_in_attn + self.tracker.num_maskmem):
                    for j in range(self.tracker.max_cond_frames_in_attn + self.tracker.max_obj_ptrs_in_encoder):
                        if is_multiplex:
                            ...
                            src = torch.randn(feat_size, b, hidden_dim, device=self.device)
                            src_pos = torch.randn(feat_size, b, hidden_dim, device=self.device)
                            prompt = torch.randn(feat_size * i + num_obj_ptr_tokens, b, mem_dim, device=self.device)
                            prompt_pos = torch.randn(feat_size * i + num_obj_ptr_tokens, b, mem_dim, device=self.device)

                            self.tracker.transformer.encoder.forward(
                                src=src, src_pos=src_pos, prompt=prompt, prompt_pos=prompt_pos, num_obj_ptr_tokens=num_obj_ptr_tokens
                            )
        ...
        for _ in tqdm(range(num_iters)):
            for i in range(1, self.max_num_kboxes + 1):
                ...
                print("Warming up masks_to_boxes with", i, f"kboxes.shape={kboxes.shape}")
                self.add_prompt(inference_state, frame_idx=start_frame_idx, text_str="cat", boxes_xywh=kboxes, box_labels=[1] * len(kboxes))
    ...
    def add_prompt(
        ...
        if boxes_xywh is not None:
            ...
            boxes_cxcywh, box_labels, geometric_prompt = self._get_visual_prompt(inference_state, frame_idx, boxes_cxcywh, box_labels)
    ...
    def forward(self, input: BatchedDatapoint, is_inference: bool = False):
        ...
        for prompt_id, prompt in zip(text_prompt_ids, text_prompt_list):
            ...
            for frame_idx, out in self.propagate_in_video(
                inference_state, start_frame_idx=0, max_frame_num_to_track=inference_state["num_frames"], reverse=False
            ):
                out_obj_ids = out["out_obj_ids"].numpy() if isinstance(out["out_obj_ids"], torch.Tensor) else out["out_obj_ids"]
                out_binary_masks = (
                    out["out_binary_masks"].numpy() if isinstance(out["out_binary_masks"], torch.Tensor) else out["out_binary_masks"]
                )
...
class Sam3MultiplexTrackingWithInteractivity(Sam3MultiplexTracking):
    def __init__(
        self, use_prev_mem_frame=False, use_stateless_refinement=False, refinement_detector_cond_frame_removal_window=30 * 4, **kwargs
    ):
        """
    ...
    def cancel_propagation(self, inference_state):
        ...
        self.add_action_history(inference_state, action_type="propagation_cancel", obj_ids=None, frame_idx=None)
    ...
    def fetch_and_process_single_frame_results(self, inference_state, frame_idx):
        ...
        out = {"obj_id_to_mask": obj_id_to_mask, "obj_id_to_score": obj_id_to_score, "obj_id_to_sam2_score": obj_id_to_sam2_score}
    ...
    def propagate_in_video(
        ...
        self.add_action_history(inference_state, action_type=propagation_type, obj_ids=obj_ids, frame_idx=start_frame_idx)
        ...
        processing_order, _end_frame_idx = self._get_processing_order(
            inference_state, start_frame_idx=start_frame_idx, max_frame_num_to_track=max_frame_num_to_track, reverse=reverse
        )
        ...
        for frame_idx in tqdm(processing_order):
            ...
            if propagation_type == "propagation_partial":
                ...
                obj_ids_local, low_res_masks_local, sam2_scores_local = self._propogate_tracker_one_frame_local_gpu(
                    tracker_states_local, frame_idx=frame_idx, reverse=reverse, run_mem_encoder=True
                )
                ...
                if self.rank == 0:
                    ...
                    suppressed_obj_ids = tracker_metadata["rank0_metadata"]["suppressed_obj_ids"][frame_idx]
                    self._cache_frame_outputs(inference_state, frame_idx, obj_id_to_mask, suppressed_obj_ids=suppressed_obj_ids)
                    suppressed_obj_ids = tracker_metadata["rank0_metadata"]["suppressed_obj_ids"][frame_idx]
                    yield (frame_idx, self._postprocess_output(inference_state, out, suppressed_obj_ids=suppressed_obj_ids))
                else:
                    yield frame_idx, DUMMY_OUTPUT  # no output for other GPUs
    ...
    def add_action_history(self, inference_state, action_type, frame_idx=None, obj_ids=None):
        ...
        propagation_actions = ["propagation_full", "propagation_partial", "propagation_fetch", "propagation_cancel"]
        ...
        action = {"type": action_type, "frame_idx": frame_idx, "obj_ids": obj_ids}
    ...
    def parse_action_history_for_propagation(self, inference_state):
        ...
        return self._parse_action_history_for_propagation(inference_state["action_history"], inference_state["num_frames"])
    ...
    def _parse_action_history_for_propagation(self, action_history, num_frames):
        ...
        if "propagation" in action_history[-1]["type"]:
            if action_history[-1]["type"] in ["propagation_fetch"]:
            ...
            elif action_history[-1]["type"] in ["propagation_partial", "propagation_full"]:
                ...
                if (
                    len(action_history) > 1 and action_history[-2]["type"] in ["propagation_partial", "propagation_full"]
                ) or action_history[-1]["frame_idx"] in [0, num_frames - 1]:
    ...
    def remove_object(self, inference_state, obj_id, frame_idx, is_user_action=False):
        ...
        if obj_rank is None:
            ...
            logger.warning(f"Object {obj_id} not found in any GPU (already removed). Skipping SAM2 state and metadata removal.")
        else:
            tracker_states_local = inference_state["sam2_inference_states"]
            if self.rank == obj_rank:
    ...
    def add_prompt(
        ...
        if points is not None:
            ...
            assert text_str is None and boxes_xywh is None, "When points are provided, text_str and boxes_xywh must be None."
    ...
    def add_sam2_new_points(
        self, inference_state, frame_idx, obj_id, points, labels, clear_old_points, rel_coordinates=True, use_prev_mem_frame=False
    ):
        ...
        if obj_rank is None:
            ...
            new_det_gpu_ids = self._assign_new_det_to_gpus(new_det_num=1, prev_workload_per_gpu=tracker_metadata["num_obj_per_gpu"])
            ...
            tracker_metadata["obj_ids_per_gpu"][obj_rank] = np.concatenate(
                [tracker_metadata["obj_ids_per_gpu"][obj_rank], np.array([obj_id], dtype=np.int64)]
            )
            tracker_metadata["num_obj_per_gpu"][obj_rank] = len(tracker_metadata["obj_ids_per_gpu"][obj_rank])
            tracker_metadata["obj_ids_all_gpu"] = np.concatenate(tracker_metadata["obj_ids_per_gpu"])
            ...
            if self.rank == obj_rank:
                tracker_states = self._get_sam2_inference_states_by_obj_ids(inference_state, [obj_id])
                assert len(tracker_states) == 1, f"[rank={self.rank}] Multiple SAM2 inference states found for the same object id."
                sam2_state = tracker_states[0]
        ...
        tracker_metadata["obj_id_to_sam2_score_frame_wise"][frame_idx][obj_id] = torch.tensor(1.0, dtype=torch.float32, device=self.device)
        ...
        if self.rank == obj_rank:
            ...
            if should_fallback_to_original_mask:
                ...
                if mask_input is None or 0 in mask_input.shape:
                    logger.warning(f"Cannot retrieve original mask input for obj_id {obj_id} at frame {frame_idx} to fallback.")
            ...
            if should_fallback_to_original_mask:
                ...
                frame_idx, obj_ids, low_res_masks, video_res_masks = self.tracker.add_new_mask(sam2_state, frame_idx, obj_id, mask_input)
            else:
            ...
            for _obj_idx in list(sam2_state["mask_inputs_per_obj"].keys()):
                ...
                _mask_only_frames = [f for f in list(sam2_state["mask_inputs_per_obj"][_obj_idx].keys()) if f not in _point_frames]
            ...
            for f in _prev_frames:
                if f in sam2_state["output_dict"].get("cond_frame_outputs", {}):
                    ...
                    _non_cond.add(f)
            sam2_state["consolidated_frame_inds"] = {"cond_frame_outputs": _cond, "non_cond_frame_outputs": _non_cond}
            self.tracker.propagate_in_video_preflight(sam2_state, run_mem_encoder=True)
        ...
        if self.rank == 0:
            obj_id_to_mask = self._build_sam2_output(
                inference_state, frame_idx, {obj_id: new_mask_data} if new_mask_data is not None else None
            )
            # post processing - remove suppressed obj_ids
            obj_id_to_score = tracker_metadata["obj_id_to_score"]
            suppressed_obj_ids = tracker_metadata["rank0_metadata"]["suppressed_obj_ids"][frame_idx]
    ...
    def _gather_obj_id_to_mask_across_gpus(self, inference_state, obj_id_to_mask_local):
        ...
        if self.world_size > 1:
            ...
            low_res_masks_peers = [
                low_res_masks_local.new_empty(num_obj, H_mask, W_mask) for num_obj in tracker_metadata["num_obj_per_gpu"]
            ]
    ...
    def _convert_low_res_mask_to_video_res(self, low_res_mask, inference_state):
        ...
        video_res_mask = F.interpolate(
            low_res_mask_3d.float(), size=(H_video, W_video), mode="bilinear", align_corners=False
        )  # (1, H_video, W_video)
    ...
    def _extract_object_to_singleton_state(self, inference_state, obj_id, obj_rank):
        ...
        singleton_consolidated_outputs = {"cond_frame_outputs": {}, "non_cond_frame_outputs": {}}
            ...
            for storage_key in ["cond_frame_outputs", "non_cond_frame_outputs"]:
                ...
                for f_idx, source_frame_out in source_outputs.items():
                    ...
                    singleton_frame_out = {
                        ...
                        "object_score_logits": source_frame_out["object_score_logits"][obj_idx_in_source : obj_idx_in_source + 1].clone(),
                    ...
                    if "obj_ptr" in source_frame_out and self.tracker.use_obj_ptrs_in_encoder:
                        ...
                        if multiplex_state is not None:
                            ...
                            singleton_frame_out["obj_ptr"] = obj_ptr_data[obj_idx_in_source : obj_idx_in_source + 1].clone()
                        else:
                            singleton_frame_out["obj_ptr"] = source_obj_ptr[obj_idx_in_source : obj_idx_in_source + 1].clone()
        ...
        new_sam2_state["output_dict_per_obj"] = {
            0: {"cond_frame_outputs": extracted_obj_cond_outputs, "non_cond_frame_outputs": extracted_obj_non_cond_outputs}
        }

        # Restore temporary outputs
        new_sam2_state["temp_output_dict_per_obj"] = {
            0: {"cond_frame_outputs": extracted_temp_cond_outputs, "non_cond_frame_outputs": extracted_temp_non_cond_outputs}
        }
        ...
        new_multiplex_state = self.tracker.multiplex_controller.get_state(
            num_valid_entries=1, device=source_state.get("device", "cuda"), dtype=torch.float32, random=False, object_ids=[obj_id]
        )
        ...
        for storage_key in ["cond_frame_outputs", "non_cond_frame_outputs"]:
            for f_idx, frame_out in singleton_consolidated_outputs[storage_key].items():
                ...
                if frame_out.get("maskmem_pos_enc") is not None:
                    frame_out["maskmem_pos_enc"] = [level.clone() if level is not None else None for level in frame_out["maskmem_pos_enc"]]
        ...
        new_sam2_state["consolidated_frame_inds"] = {"cond_frame_outputs": set(), "non_cond_frame_outputs": set()}
