...
from sam3.model.sam3_tracker_utils import get_1d_sine_pe, get_next_point, sample_box_points, select_closest_cond_frames
...
class VideoTrackingMultiplex(nn.Module):
    ...
    def _build_sam_heads(self):
        ...
        self.sam_mask_decoder = MultiplexMaskDecoder(
            ...
            transformer=TwoWayTransformer(depth=2, embedding_dim=self.hidden_dim, mlp_dim=2048, num_heads=8),
        ...
        *,
        ...
        if is_interactive:
            ...
            if self.add_output_suppression_embeddings:
                ...
                output_merged_embed = valid_object_mask * output_valid_embed + (1 - valid_object_mask) * output_invalid_embed
            else:
        ...
        if self.use_obj_ptrs_in_encoder:
            ...
            if self.pred_obj_scores and self.use_no_obj_ptr:
                ...
                if self.use_linear_no_obj_ptr:
                    obj_ptr = lambda_is_obj_appearing * obj_ptr + (1 - lambda_is_obj_appearing) * self.no_obj_ptr_linear(obj_ptr)
                else:
                    if self.fixed_no_obj_ptr:
    ...
    def forward_image(
        self, img_batch, *, need_sam3_out: bool = False, need_interactive_out: bool = False, need_propagation_out: bool = False
    ):
        ...
        if self.share_necks:
            ...
            backbone_out = self.backbone.forward_image(img_batch, need_sam3_out=need_sam3_out, need_sam2_out=need_propagation_out)
            backbone_out["interactive"] = backbone_out["sam2_backbone_out"]
        else:
            backbone_out = self.backbone.forward_image(
                img_batch, need_sam3_out=need_sam3_out, need_interactive_out=need_interactive_out, need_propagation_out=need_propagation_out
            )
        ...
        for out_type in backbone_out.keys():
            for i in range(len(backbone_out[out_type]["backbone_fpn"])):
                backbone_out[out_type]["backbone_fpn"][i].tensors = self._maybe_clone(backbone_out[out_type]["backbone_fpn"][i].tensors)
                backbone_out[out_type]["vision_pos_enc"][i] = self._maybe_clone(backbone_out[out_type]["vision_pos_enc"][i])
        return backbone_out
    ...
    def _prepare_prompt_inputs_meta(self, backbone_out, input, start_frame_idx=0):
        ...
        if num_init_cond_frames == 1:
            ...
            init_cond_frames = [start_frame_idx] + self.rng.choice(
                range(start_frame_idx + 1, num_frames), num_init_cond_frames - 1, replace=False
            ).tolist()
            ...
            if not use_pt_input:
                ...
                if use_box_input:
                    ...
                    points, labels = get_next_point(
                        gt_masks=gt_masks_per_frame[t], pred_masks=None, method=("uniform" if self.training else self.pt_sampling_for_eval)
                    )
    ...
    def _prepare_backbone_features_per_frame(
        self, img_batch, img_ids, *, need_interactive_out: bool = False, need_propagation_out: bool = False
    ):
        ...
        *,
        ...
        if not is_init_cond_frame and use_prev_mem_frame:
            ...
            selected_cond_outputs, unselected_cond_outputs = select_closest_cond_frames(
                frame_idx, cond_outputs, self.max_cond_frames_in_attn, keep_first_cond_frame=self.keep_first_cond_frame
            )
    ...
    def _encode_new_memory(
        ...
        *,
        ...
        if self.apply_sigmoid_to_mask_logits_for_mem_enc:
            ...
            assert not self.binarize_mask_from_pts_for_mem_enc, "haven't been trained this way; beware of hardcoded config override"
        ...
        if self.condition_as_mask_input:
            ...
            cond_values = torch.full((num_objects,), self.condition_as_mask_input_bg, device=mask_for_mem.device, dtype=mask_for_mem.dtype)
    ...
    def forward_tracking(self, backbone_out, input, return_dict=False, objects_to_interact: Optional[list[int]] = None):
        ...
        output_dict = {"cond_frame_outputs": cond_frame_outputs, "non_cond_frame_outputs": non_cond_frame_outputs}
        ...
        for stage_id in processing_order:
            ...
            if img_feats_already_computed:
                ...
                (current_image, current_backbone_features) = self._prepare_backbone_features_per_frame(
                    input.img_batch, img_ids, need_interactive_out=need_interactive_out, need_propagation_out=True
                )
    ...
    def _track_step_aux(
        ...
        *,
        ...
        """
        current_out: StageOutput = {"conditioning_objects": set(), "point_inputs": point_inputs, "mask_inputs": mask_inputs}
        ...
        if mask_inputs is not None:
            ...
            if prev_sam_mask_logits is not None:
                assert objects_to_interact is not None, "objects_to_interact must be specified when refining with prev_sam_mask_logits"
        ...
        if mode == "mask_as_output":
            ...
            if mode in ["interaction_only", "propagation_and_interaction"]:
                ...
                interaction_out = self._forward_sam_heads(
                    ...
                    multimask_output=multimask_output,
                    objects_to_interact=(
                        objects_to_interact if objects_to_interact is not None else list(range(multiplex_state.total_valid_entries))
                    ),
                    multiplex_state=multiplex_state,
                )
        ...
        if frame_idx in frames_to_add_correction_pt:
            ...
            if self.training:
                ...
                if interact_with_all_objects:
                    ...
                    num_objects_to_correct = self.rng2.integers(1, int(gt_masks.shape[0] * self.ratio_of_objects_to_correct_for_train) + 1)
                else:
                    num_objects_to_correct = max(1, int(gt_masks.shape[0] * self.ratio_of_objects_to_correct_for_train))

                objects_to_interact = self.rng2.choice(range(gt_masks.shape[0]), size=num_objects_to_correct, replace=False).tolist()
            ...
            for _ in range(self.num_correction_pt_per_frame):
                ...
                assert low_res_masks.shape[0] > max(objects_to_interact), f"interacting {objects_to_interact} in {low_res_masks.shape}?"
                ...
                if torch.is_floating_point(interact_low_res_masks) and interact_low_res_masks.dtype != low_res_masks.dtype:
                ...
                if torch.is_floating_point(interact_high_res_masks) and interact_high_res_masks.dtype != high_res_masks.dtype:
                ...
                if torch.is_floating_point(interact_low_res_multimasks) and interact_low_res_multimasks.dtype != low_res_multimasks.dtype:
    ...
    def _trim_output_and_memory(
        self, frame_idx: int, output_dict: dict[str, dict[int, StageOutput]], current_out: StageOutput, memory_encoder_was_used: bool
    ) -> StageOutput:
        ...
        if self.trim_past_non_cond_mem_for_eval and not self.training:
            ...
            if past_out is not None:
                if (self.use_memory_selection and past_out.get("eff_iou_score", 0) < self.mf_threshold) or not self.use_memory_selection:
    ...
    def track_step(
        ...
        *,
        ...
        current_out = self._trim_output_and_memory(frame_idx, output_dict, current_out, memory_encoder_was_used=run_mem_encoder)
    ...
    def _compile_all_components(self):
        ...
        self.maskmem_backbone.forward = torch.compile(self.maskmem_backbone.forward, mode="max-autotune", fullgraph=True, dynamic=False)
    ...
    def frame_filter(self, output_dict, track_in_reverse, frame_idx, num_frames, r):
        ...
        for i in range(start, end, step):
            if i not in output_dict["non_cond_frame_outputs"] or "eff_iou_score" not in output_dict["non_cond_frame_outputs"][i]:
                continue
...
def _merge(d1: StageOutput, d2: SAMOutput, k1: str, k2: str, d2_idx: list[int], strict: bool = True):
    if strict:
        assert k1 in d1, f"{k1} not found"
    else:
        if k1 not in d1:
...
class VideoTrackingDynamicMultiplex(VideoTrackingMultiplex):
    ...
    def prepare_prompt_inputs(self, backbone_out, input, start_frame_idx=0):
        ...
        **Transition points**
        ...
        if self.training and self.enable_dynamic_training:
            ...
            transition_points = self.rng2.choice(available_transition_points, num_transition_points, replace=False).tolist()
            ...
            for stage_id in range(start_frame_idx + 1, num_frames):
                if stage_id in transition_points:
                    ...
                    new_idx_per_transition[stage_id] = list(
                        range(len(object_appearance_order), len(object_appearance_order) + len(stage_objects))
                    )
                ...
                if stage_id in init_cond_frames:
                    ...
                    raise ValueError(f"Unexpected {stage_id=}? {init_cond_frames=} {frames_not_in_init_cond=} {transition_points=}")
            ...
            for stage_id in range(start_frame_idx, num_frames):
                if stage_id in transition_points:
                    ...
                    new_idx_per_transition[stage_id] = list(range(num_objects_before, num_objects_before + len(new_objects)))
        ...
        for stage_id in range(start_frame_idx, num_frames):
            gt_masks_per_frame[stage_id] = gt_masks_per_frame[stage_id][object_appearance_order][valid_idx_per_frame[stage_id]]
    ...
    def add_new_masks_to_existing_state(
        ...
        *,
        ...
        new_object_idx = multiplex_state.find_next_batch_of_available_indices(
            num_objects=num_new_objects, allow_new_buckets=allow_new_buckets, prefer_new_buckets=prefer_new_buckets
        )
    ...
    def track_step(
        ...
        *,
        ...
        current_out = self._trim_output_and_memory(
            frame_idx=frame_idx, output_dict=output_dict, current_out=current_out, memory_encoder_was_used=run_mem_encoder
        )
    ...
    def forward_tracking(self, backbone_out, input, return_dict=False, objects_to_interact: Optional[list[int]] = None):
        ...
        for stage_id in processing_order:
            ...
            if img_feats_already_computed:
                ...
                need_interactive_out = (
                    (stage_id in frames_to_add_correction_pt) or (stage_id in init_cond_frames) or (stage_id in transition_points)
                )
                (current_image, current_backbone_features) = self._prepare_backbone_features_per_frame(
                    input.img_batch, img_ids, need_interactive_out=need_interactive_out, need_propagation_out=True
                )
        ...
        if self.is_dynamic_vos_evaluation:
            ...
            all_frame_outputs = [all_frame_outputs[t] for t in range(num_frames)]
        # Make DDP happy with activation checkpointing by removing unused keys
        all_frame_outputs = [{k: v for k, v in d.items() if k != "obj_ptr"} if d is not None else None for d in all_frame_outputs]
        ...
        if self.is_dynamic_vos_evaluation:
            ...
            for stage_i, frame_out in enumerate(all_frame_outputs):
                if frame_out is None:
                    all_frame_outputs[stage_i] = {"pred_masks": torch.zeros((num_objects, *shape), device=device, dtype=dtype)}
                    continue
                ...
                if pred_mask.shape[0] < num_objects:
                    ...
                    frame_out["pred_masks"] = torch.cat(
                        [pred_mask, torch.zeros((num_objects - pred_mask.shape[0], *shape), device=device, dtype=dtype)], dim=0
