...
class SAM3VLBackbone(nn.Module):
    ...
    def forward_image(self, samples: torch.Tensor):
        return activation_ckpt_wrapper(self._forward_image_no_act_ckpt)(
            samples=samples, act_ckpt_enable=self.act_ckpt_whole_vision_backbone and self.training
        )
    ...
    def _forward_text_no_ack_ckpt(self, captions, input_boxes=None, additional_text=None, device="cuda"):
        output = {}
        ...
        sdpa_context = sdpa_kernel([SDPBackend.MATH, SDPBackend.EFFICIENT_ATTENTION, SDPBackend.FLASH_ATTENTION])

        with sdpa_context:
            text_attention_mask, text_memory, text_embeds = self.language_backbone(text_to_encode, input_boxes, device=device)
...
class SAM3VLBackboneTri(SAM3VLBackbone):
    ...
    def forward_image(self, samples, *, need_sam3_out: bool = True, need_interactive_out: bool = True, need_propagation_out: bool = True):
    ...
    def _forward_image_tri_no_act_ckpt(self, samples, need_sam3_out=True, need_interactive_out=True, need_propagation_out=True):
        (sam3_features, sam3_pos, interactive_features, interactive_pos, propagation_features, propagation_pos) = (
            self.vision_backbone.forward(
                samples, need_sam3_out=need_sam3_out, need_interactive_out=need_interactive_out, need_propagation_out=need_propagation_out
            )
        )
        if self.scalp > 0:
            sam3_features, sam3_pos = (sam3_features[: -self.scalp], sam3_pos[: -self.scalp])
            interactive_features, interactive_pos = (interactive_features[: -self.scalp], interactive_pos[: -self.scalp])
            propagation_features, propagation_pos = (propagation_features[: -self.scalp], propagation_pos[: -self.scalp])
...
class VisionOnly(nn.Module):
    ...
    def _compile(self):
        if self.should_compile and not self.compiled:
            self.vision_backbone = torch.compile(self.vision_backbone, mode=self.compile_mode, **self.compile_extra_args)
            self.compiled = True
    ...
    def forward_image(self, samples):
        ...
        output = {"vision_features": src, "vision_mask": mask, "vision_pos_enc": pos, "backbone_fpn": features}
        return output
...
class TriHeadVisionOnly(VisionOnly):
    ...
    def forward_image(self, samples, *, need_sam3_out: bool = True, need_interactive_out: bool = True, need_propagation_out: bool = True):
        self._compile()
        # Forward through backbone
        (sam3_features, sam3_pos, interactive_features, interactive_pos, propagation_features, propagation_pos) = self.vision_backbone(
            samples, need_sam3_out=need_sam3_out, need_interactive_out=need_interactive_out, need_propagation_out=need_propagation_out
        )
