...
class Sam3MultiplexImageBase(Sam3Image):
    ...
    def __init__(self, *args, tracking_score_thresh: float = 0.0, offload_outputs_to_cpu_for_eval: bool = False, **kwargs):
    ...
    def forward(
        ...
        for frame_idx in range(num_frames):
            ...
            cur_out, _ = self.forward_video_grounding(
                backbone_out=backbone_out, find_input=find_input, find_target=find_target, geometric_prompt=geometric_prompt
            )
    ...
    def forward_video_grounding(self, backbone_out, find_input, find_target, geometric_prompt: Prompt, **kwargs):
    ...
    def _batch_find_inputs(self, find_inputs: List[FindStage], chunk_start: int, chunk_end: int) -> FindStage:
        """
        ...
        batched_input_boxes_mask = torch.cat(input_boxes_mask_list, dim=0) if input_boxes_mask_list[0] is not None else None

        # Concatenate input_boxes_label
        input_boxes_label_list = [fi.input_boxes_label for fi in chunk_find_inputs]
        batched_input_boxes_label = torch.cat(input_boxes_label_list, dim=0) if input_boxes_label_list[0] is not None else None
        ...
        batched_input_points_mask = torch.cat(input_points_mask_list, dim=0) if input_points_mask_list[0] is not None else None
    ...
    def _batch_geometric_prompts(self, geometric_prompts: List[Prompt], chunk_start: int, chunk_end: int) -> Prompt:
        """
...
class Sam3MultiplexDetector(Sam3MultiplexImageBase):
    def __init__(self, *args, async_all_gather=True, gather_backbone_out=None, is_multiplex=False, **kwargs):
        super().__init__(*args, **kwargs)
        ...
        **kwargs,
        ...
        if max_frame_num_to_track is not None:
            ...
            if track_in_reverse:
                ...
                valid_frame_start = max(0, propagate_in_video_start_frame_idx - max_frame_num_to_track + 1)
                valid_frame_end = num_frames
            else:
                # When going forwards, limit how far ahead we can go from current frame
                valid_frame_start = 0
                valid_frame_end = min(num_frames, propagate_in_video_start_frame_idx + max_frame_num_to_track)
        else:
        ...
        for k, (v, handle) in multigpu_buffer[frame_idx].items():
            if self.is_multiplex:
                if (k.startswith("interactive_backbone_") or k.startswith("propagation_backbone_")) and not return_sam2_backbone_feats:
                    continue
            else:
    ...
    def _build_multigpu_buffer_next_chunk(
        ...
        for rank in range(self.world_size):
            ...
            if self.gather_backbone_out:
                ...
                if self.is_multiplex:
                    frame_buffer["interactive_backbone_fpn_0"] = (inte_fpn0[rank], inte_fpn_handle0)
                    frame_buffer["interactive_backbone_fpn_1"] = (inte_fpn1[rank], inte_fpn_handle1)
                    frame_buffer["interactive_backbone_fpn_2"] = (inte_fpn2[rank], inte_fpn_handle2)
                    frame_buffer["interactive_backbone_pos_enc"] = (inte_vision_pos_enc, None)
    ...
    def _process_grounding_chunk_batched(
        ...
        chunk_geo_prompts = [self._get_geo_prompt_from_find_input(find_inputs[i % len(find_inputs)]) for i in range(chunk_start, chunk_end)]
    ...
    def _slice_batched_output(self, chunk_outputs, local_idx: int, return_sam2_backbone_feats: bool):
        """
        ...
        # Keys to skip
        skip_keys = {"_chunk_size", "_interactive_backbone", "_sam2_backbone", "prev_encoder_out", "encoder_hidden_states", "aux_outputs"}
    ...
    def _cleanup_previous_chunks_multigpu(
        self, grounding_cache, current_chunk_key, batch_size: int, num_frames: int, track_in_reverse: bool
    ):
