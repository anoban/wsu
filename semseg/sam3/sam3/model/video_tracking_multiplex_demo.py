...
from sam3.model.video_tracking_multiplex import NO_OBJ_SCORE, VideoTrackingDynamicMultiplex, concat_points
...
class VideoTrackingMultiplexDemo(VideoTrackingDynamicMultiplex):
    ...
    def init_state(
        self, video_path, offload_video_to_cpu, offload_state_to_cpu, async_loading_frames=False, use_torchcodec=False, use_cv2=False
    ):
        ...
        if not self.apply_sigmoid_to_mask_logits_for_mem_enc:
            raise NotImplementedError("Multi-object tracking requires sigmoid in memory encoder for non-overlapping constraints.")
    ...
    def _obj_id_to_idx(self, inference_state, obj_id, error_if_new=False):
        ...
        if (self.is_dynamic_model or not inference_state["tracking_has_started"]) and not error_if_new:
            ...
            raise RuntimeError(f"Cannot add new object id {obj_id}. All existing object ids: {inference_state['obj_ids']}.")
    ...
    def _extract_object_for_interaction(self, inference_state, obj_id, frame_idx):
                    ...
                    if has_valid_data:
                            ...
                            if multiplex_state is not None:
                                ...
                                if (
                                    ...
                                    try:
                                        ...
                                        logging.warning(
                                            "[EXTRACT] demux failed for maskmem_features shape %s: %s", tuple(maskmem_features.shape), exc
                                        )
                                        demuxed_features = None
                                    if demuxed_features is not None:
                                        maskmem_features = demuxed_features[obj_idx_in_source : obj_idx_in_source + 1].clone()
                                    else:
                                ...
                                if multiplex_state is not None:
                                    ...
                                    if (
                                        ...
                                        try:
                                            ...
                                            logging.warning(
                                                "[EXTRACT] demux failed for maskmem_pos_enc level shape %s: %s", tuple(level_enc.shape), exc
                                            )
                                            demuxed_level = None
                                        if demuxed_level is not None:
                                            remapped_pos_enc.append(demuxed_level[obj_idx_in_source : obj_idx_in_source + 1].clone())
                                        elif level_enc.shape[0] >= obj_idx_in_source + 1:
                                            remapped_pos_enc.append(level_enc[obj_idx_in_source : obj_idx_in_source + 1].clone())
                                        else:
                                            logging.warning(
                        ...
                        if "obj_ptr" in source_frame_out and self.use_obj_ptrs_in_encoder:
                            ...
                            if multiplex_state is not None:
                                ...
                                singleton_frame_out["obj_ptr"] = obj_ptr_data_space[obj_idx_in_source : obj_idx_in_source + 1].clone()
                            else:
        ...
        remaining_obj_ids, _ = self.remove_object(source_state, obj_id, strict=False, need_output=False, clear_user_refined_map=False)
        ...
        if updated_multiplex_state is not None:
            if getattr(updated_multiplex_state, "assignments", None) is None or updated_multiplex_state.total_valid_entries == 0:
        ...
        new_multiplex_state = self.multiplex_controller.get_state(
            num_valid_entries=1, device=source_state["device"], dtype=torch.float32, random=False, object_ids=[obj_id]
        )
    ...
    @torch.inference_mode()
    def _merge_singleton_interaction_result(self, inference_state, singleton_state, obj_id, original_obj_idx):
        """
        ...
        inference_state["output_dict_per_obj"][new_obj_idx] = {"cond_frame_outputs": {}, "non_cond_frame_outputs": {}}
        ...
        total_valid_entries = (
            getattr(multiplex_state, "total_valid_entries", 0) if multiplex_state is not None and assignments is not None else 0
        )
        ...
        if not need_state_reinit and getattr(multiplex_state, "object_ids", None):
            if obj_id in multiplex_state.object_ids:
                ...
                total_valid_entries = getattr(multiplex_state, "total_valid_entries", 0) if assignments is not None else 0
        ...
        if "point_inputs_per_obj" in singleton_state and singleton_obj_idx in singleton_state["point_inputs_per_obj"]:
            if "point_inputs_per_obj" not in inference_state:
                inference_state["point_inputs_per_obj"] = {}
            inference_state["point_inputs_per_obj"][new_obj_idx] = singleton_state["point_inputs_per_obj"][singleton_obj_idx].copy()
        ...
        if "mask_inputs_per_obj" in singleton_state and singleton_obj_idx in singleton_state["mask_inputs_per_obj"]:
            if "mask_inputs_per_obj" not in inference_state:
                inference_state["mask_inputs_per_obj"] = {}
            inference_state["mask_inputs_per_obj"][new_obj_idx] = singleton_state["mask_inputs_per_obj"][singleton_obj_idx].copy()
        ...
        if "output_dict_per_obj" in singleton_state and singleton_obj_idx in singleton_state["output_dict_per_obj"]:
            if "output_dict_per_obj" not in inference_state:
                inference_state["output_dict_per_obj"] = {}
            inference_state["output_dict_per_obj"][new_obj_idx] = singleton_state["output_dict_per_obj"][singleton_obj_idx].copy()

        if "temp_output_dict_per_obj" in singleton_state and singleton_obj_idx in singleton_state["temp_output_dict_per_obj"]:
            if "temp_output_dict_per_obj" not in inference_state:
                inference_state["temp_output_dict_per_obj"] = {}
            inference_state["temp_output_dict_per_obj"][new_obj_idx] = singleton_state["temp_output_dict_per_obj"][singleton_obj_idx].copy()
        ...
        if "output_dict" in singleton_state:
            ...
            for singleton_storage_key in ["cond_frame_outputs", "non_cond_frame_outputs"]:
                    ...
                    if frame_idx not in inference_state["output_dict"][singleton_storage_key]:
                        ...
                        if singleton_frame_out.get("maskmem_features") is not None and multiplex_state is not None:
                            ...
                            maskmem_features_data = torch.zeros(
                                feature_shape, dtype=singleton_features_data.dtype, device=singleton_features_data.device
                            )
                        ...
                        if singleton_frame_out.get("maskmem_pos_enc") is not None and multiplex_state is not None:
                            ...
                            for level_enc in singleton_frame_out["maskmem_pos_enc"]:
                                ...
                                level_tensor = torch.zeros(level_shape, dtype=level_data.dtype, device=level_data.device)
                        ...
                        inference_state["output_dict"][singleton_storage_key][frame_idx] = {
                            ...
                            "conditioning_objects": (
                                set([new_obj_idx]) if singleton_obj_idx in singleton_frame_out.get("conditioning_objects", set()) else set()
                            ),
                            "pred_masks": torch.zeros(
                                (num_objs, 1, singleton_frame_out["pred_masks"].shape[2], singleton_frame_out["pred_masks"].shape[3]),
                        ...
                        inference_state["output_dict"][singleton_storage_key][frame_idx]["pred_masks"][new_obj_idx : new_obj_idx + 1] = (
                        ...
                        if "pred_masks_video_res" in singleton_frame_out:
                            inference_state["output_dict"][singleton_storage_key][frame_idx]["pred_masks_video_res"] = torch.zeros(
                                (
                                    num_objs,
                                    1,
                        ...
                        if "obj_ptr" in singleton_frame_out and self.use_obj_ptrs_in_encoder:
                            ...
                            inference_state["output_dict"][singleton_storage_key][frame_idx]["obj_ptr"] = multiplex_state.mux(obj_ptr_data)
                    else:
                        ...
                        if singleton_frame_out.get("maskmem_features") is not None and multiplex_state is not None:
                            ...
                            if existing_features_muxed is not None:
                                ...
                                if existing_features_muxed.shape[:2] == (multiplex_state.num_buckets, multiplex_state.multiplex_count):
                        ...
                        if singleton_frame_out.get("maskmem_pos_enc") is not None and multiplex_state is not None:
                            ...
                            new_maskmem_pos_enc = []
                            max_levels = max(len(singleton_frame_out["maskmem_pos_enc"]), len(existing_pos_enc_list))
                            for level_idx in range(max_levels):
                                ...
                                existing_level_muxed = existing_pos_enc_list[level_idx] if level_idx < len(existing_pos_enc_list) else None
                                ...
                                if existing_level_muxed is not None:
                                    ...
                                    if existing_level_muxed.shape[:2] == (multiplex_state.num_buckets, multiplex_state.multiplex_count):
                                ...
                                if existing_level_data is None:
                                    ...
                                    existing_level_data = torch.zeros(
                                        level_shape, dtype=singleton_level_data.dtype, device=singleton_level_data.device
                                    )
                        ...
                        if new_obj_idx >= num_existing_objs:
                            ...
                            main_frame_out["pred_masks"] = torch.cat(
                                [
                                    ...
                                    torch.zeros(
                                        (pad_size, 1, singleton_pred_masks.shape[2], singleton_pred_masks.shape[3]),
                            ...
                            main_frame_out["object_score_logits"] = torch.cat(
                                [
                                    ...
                                    torch.full((pad_size, 1), NO_OBJ_SCORE, dtype=singleton_scores.dtype, device=singleton_scores.device),
                                ],
                                dim=0,
                            )
                        ...
                        # Handle obj_ptr
                        if "obj_ptr" in singleton_frame_out and self.use_obj_ptrs_in_encoder:
                            singleton_obj_ptr_data = singleton_multiplex_state.demux(singleton_frame_out["obj_ptr"])  # [1, D]
                                    ...
                                    if main_obj_ptr_data.shape[0] < new_obj_idx + 1:
                                        ...
                                        main_obj_ptr_data = torch.cat(
                                            [
                                                main_obj_ptr_data,
                                                torch.zeros(
                                                    (pad_size, main_obj_ptr_data.shape[1]),
    ...
    def add_new_points(
        ...
        if is_new_state:
            multiplex_state = self.multiplex_controller.get_state(
                num_valid_entries=1, device=inference_state["device"], dtype=torch.float32, random=False, object_ids=obj_ids
            )
    ...
    def add_new_masks(
        ...
        with torch.profiler.record_function("add_new_masks.obj_loop"):
            ...
            for obj_idx2, obj_temp_output_dict2 in temp_output_dict_per_obj.items():
                ...
                current_out2["pred_masks_video_res"] = torch.where(suppress_mask, NO_OBJ_SCORE, current_out2["pred_masks_video_res"])
    ...
    def _consolidate_temp_output_across_obj(self, inference_state, frame_idx, is_cond, run_mem_encoder, consolidate_at_video_res=False):
        ...
        consolidated_out = {
            ...
            consolidated_mask_key: torch.full(
                size=(consolidated_batch_size, 1, consolidated_H, consolidated_W),  # Use consolidated_batch_size, not batch_size!
        ...
        if need_to_reconstruct_from_per_obj and consolidated_mask_key not in consolidated_out:
            ...
            consolidated_out[consolidated_mask_key] = torch.zeros(
                (consolidated_batch_size, 1, consolidated_H, consolidated_W), dtype=torch.float32, device=inference_state["storage_device"]
            )
            consolidated_out["object_score_logits"] = torch.full(
                (consolidated_batch_size, 1), NO_OBJ_SCORE, dtype=torch.float32, device=inference_state["storage_device"]
            )
            ...
            if obj_idx >= consolidated_pred_masks.shape[0]:
                ...
                consolidated_pred_masks = torch.cat(
                    [
                        ...
                        torch.zeros(
                            (pad_size, 1, consolidated_pred_masks.shape[-2], consolidated_pred_masks.shape[-1]),
                ...
                if "object_score_logits" in consolidated_out:
                    ...
                    consolidated_scores = torch.cat(
                        [
                            consolidated_scores,
                            torch.full((pad_size, 1), NO_OBJ_SCORE, dtype=consolidated_scores.dtype, device=consolidated_scores.device),
                        ],
                        dim=0,
                    )
        ...
        if need_to_reconstruct_from_per_obj:
            ...
            if obj_score_logits_list:
                ...
                device = inference_state["device"]
                consolidated_out["object_score_logits"] = torch.zeros((batch_size, 1), dtype=torch.float32, device=device)
    ...
    def propagate_in_video(
        self, inference_state, start_frame_idx, max_frame_num_to_track, reverse, tqdm_disable=False, obj_ids=None, run_mem_encoder=True
    ):
    ...
    @torch.inference_mode()
    def clear_all_points_in_frame(self, inference_state, frame_idx, obj_id, need_output=True, preserve_user_refined: bool = False):
    ...
    def _get_image_feature(self, inference_state, frame_idx, batch_size):
        ...
        if backbone_out is None:
            ...
            backbone_out = self.forward_image(
                NestedTensor(tensors=image, mask=None), need_sam3_out=True, need_interactive_out=True, need_propagation_out=True
            )
        ...
        if add_to_existing_state or reconditioning:
            with torch.profiler.record_function("VideoTrackingMultiplexDemo.add_new_masks_to_existing_state"):
                ...
                interactive_high_res_features = [
                    ...
                    for x, s in zip(backbone_features_interactive["vision_feats"][:-1], backbone_features_interactive["feat_sizes"][:-1])
                ]
    ...
    @torch.inference_mode()
    def remove_object(self, inference_state, obj_id: int, strict=False, need_output=True, clear_user_refined_map: bool = True):
        """
        ...
        return self.remove_objects(
            inference_state, obj_ids=[obj_id], strict=strict, need_output=need_output, clear_user_refined_map=clear_user_refined_map
        )

    @torch.inference_mode()
    def remove_objects(self, inference_state, obj_ids: Iterable[int], strict=False, need_output=True, clear_user_refined_map: bool = True):
        """
        ...
        for old_obj_idx_to_rm, obj_id in zip(old_obj_idxs_to_rm, obj_ids, strict=True):
            ...
            for frame_idx in obj_input_frames_inds:
                self.clear_all_points_in_frame(
                    inference_state, frame_idx, obj_id, need_output=False, preserve_user_refined=not clear_user_refined_map
                )
        ...
        def _slice_state(output_dict, storage_key):
            for frame_idx, out in output_dict[storage_key].items():
                ...
                local_remain_old_obj_inds = [obj_idx for obj_id, obj_idx in local_obj_id_to_idx.items() if obj_id not in obj_ids]
                ...
                keep_indices = [idx for idx in local_remain_old_obj_inds if 0 <= idx < max_pred and 0 <= idx < max_scores]
    ...
    def warm_up_compilation(self, offload_video_to_cpu=False, offload_state_to_cpu=False):
        ...
        raise NotImplementedError("Please use `VideoTrackingMultiplexDemoPerBucketInference` instead for full model compilation.")


class Sam3VideoTrackingMultiplexDemo(VideoTrackingMultiplexDemo):
    @torch.inference_mode()
    def init_state(
        self, video_height, video_width, num_frames, cached_features=None, offload_video_to_cpu=False, offload_state_to_cpu=False
    ):
        """Initialize a inference state."""
        # Make sure that sigmoid is used on mask logits (should be True for all our recent models).
        # Since we rely on large negative values as scores for missing objects, the raw logits
    ...
    def _apply_object_wise_non_overlapping_constraints(self, pred_masks, obj_scores, background_value=-10.0):
        ...
        pred_masks = torch.where(pixel_level_non_overlapping_masks > 0, pred_masks, torch.clamp(pred_masks, max=background_value))
        return pred_masks
