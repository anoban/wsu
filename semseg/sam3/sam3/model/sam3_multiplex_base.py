...
class Sam3MultiplexTrackerPredictor(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        raise NotImplementedError("Use the sam2 predictor APIs instead. Check VideoTrackingWithPromptDemo class for details.")
...
class Sam3MultiplexBase(Sam3VideoBase):
    def __init__(
        ...
        **kwargs,
        ...
        self.suppress_overlapping_based_on_recent_occlusion_threshold = suppress_overlapping_based_on_recent_occlusion_threshold
        ...
        logger.info(f"`setting max_num_objects` to {max_num_objects} -- creating {num_obj_for_compile=} objects for torch.compile cache")
    ...
    def _det_track_one_frame_impl(
        ...
        if tracker_obj_scores_global.shape[0] > 0:
            ...
            tracker_metadata_new["obj_id_to_sam2_score_frame_wise"][frame_idx].update(dict(zip(sam2_obj_ids, tracker_obj_scores_global)))
    ...
    def run_backbone_and_detection(
        ...
        det_out = {"bbox": pred_boxes_xyxy, "mask": pred_masks, "scores": pred_probs}
        ...
        with torch.profiler.record_function("run_backbone_and_detection.feature_cache"):
            feature_cache[frame_idx] = (input_batch.img_batch.tensors[frame_idx], backbone_cache)
    ...
    def run_tracker_propagation(
        self, frame_idx: int, num_frames: int, reverse: bool, tracker_states_local: List[Any], tracker_metadata_prev: Dict[str, np.ndarray]
    ):
        ...
        with torch.profiler.record_function("all_gather_low_res_masks_local"):
            ...
            if self.world_size > 1:
                ...
                low_res_masks_peers = [
                    low_res_masks_local.new_empty(num_obj, H_mask, W_mask) for num_obj in tracker_metadata_prev["num_obj_per_gpu"]
                ]
                obj_scores_peers = [obj_scores_local.new_empty(num_obj) for num_obj in tracker_metadata_prev["num_obj_per_gpu"]]
    ...
    def _recondition_masklets(
        ...
        obj_ids_all_gpu_t = torch.from_numpy(tracker_metadata["obj_ids_all_gpu"]).to(device=tracker_obj_scores_global.device)
        ...
        new_masks_binary = (
            F.interpolate(new_masks.unsqueeze(1), size=(input_mask_res, input_mask_res), mode="bilinear", align_corners=False).squeeze(1)
            > 0
        )  # (K, H, W)
                ...
                for inference_state in tracker_states_local:
                    if trk_obj_id in inference_state["obj_ids"]:
                        self.tracker.add_new_mask(
                            inference_state=inference_state, frame_idx=frame_idx, obj_id=trk_obj_id, mask=new_masks_binary[i]
                        )
                        reconditioned_obj_ids.update(inference_state["obj_idx_to_id"].values())
                        break
    ...
    def run_tracker_update_planning_phase(
        ...
        if self.reconstruction_bbox_iou_thresh > 0 and len(adt_result.trk_id_to_max_iou_high_conf_det) > 0:
            with torch.profiler.record_function("evaluate_reconstruction_bbox_iou_thresh"):
                ...
                trk_ids = [sam2_obj_ids_all_gpu.index(trk_obj_id) for trk_obj_id in trk_obj_ids if trk_obj_id in sam2_obj_ids_all_gpu]
                ...
                sam2_box_normalized = sam2_box_pixels / torch.tensor(
                    [mask_width, mask_height, mask_width, mask_height], device=sam2_box_pixels.device
                )
                iou = fast_diag_box_iou(det_boxes_bbox_iou, sam2_box_normalized)[0]
                if iou < self.reconstruction_bbox_iou_thresh and torch.any(det_scores_bbox_iou >= self.reconstruction_bbox_det_score):
        ...
        if batch_size > 0:
            ...
            with torch.profiler.record_function("_tracker_update_memories"):
                self._tracker_update_memories(
                    tracker_states_local, frame_idx, tracker_metadata=tracker_metadata_prev, low_res_masks=tracker_low_res_masks_global
                )
        ...
        if len(new_det_obj_ids) > 0:
            ...
            tracker_metadata_new["obj_id_to_score"].update(zip(new_det_obj_ids, det_scores_np[adt_result.new_det_fa_inds]))
            ...
            tracker_metadata_new["obj_id_to_sam2_score_frame_wise"][frame_idx].update(zip(new_det_obj_ids, new_det_scores_tensor))
            ...
            if num_new > 0:
                with torch.profiler.record_function("extend_gpu_metadata_for_new_objects"):
                    ...
                    gpu_meta["obj_first_frame"] = torch.cat(
                        [
                            gpu_meta.get("obj_first_frame", torch.empty(0, dtype=torch.long, device=device)),
                            torch.full((num_new,), frame_idx, dtype=torch.long, device=device),
                        ]
                    )
                    gpu_meta["consecutive_unmatch_count"] = torch.cat(
                        [
                            gpu_meta.get("consecutive_unmatch_count", torch.empty(0, dtype=torch.long, device=device)),
                            torch.zeros(num_new, dtype=torch.long, device=device),
                        ]
                    )
                    gpu_meta["trk_keep_alive"] = torch.cat(
                        [
                            gpu_meta.get("trk_keep_alive", torch.empty(0, dtype=torch.long, device=device)),
                            torch.full((num_new,), self.init_trk_keep_alive, dtype=torch.long, device=device),
                        ]
                    )
                    gpu_meta["removed_mask"] = torch.cat(
                        [
                            gpu_meta.get("removed_mask", torch.empty(0, dtype=torch.bool, device=device)),
                            torch.zeros(num_new, dtype=torch.bool, device=device),
                        ]
                    )
                    gpu_meta["last_occluded_tensor"] = torch.cat(
                        [
                            gpu_meta.get("last_occluded_tensor", torch.empty(0, dtype=torch.long, device=device)),
                            torch.full((num_new,), NEVER_OCCLUDED, dtype=torch.long, device=device),
                        ]
                    )
                    ...
                    new_N = old_N + num_new
                    old_overlap = gpu_meta.get("overlap_pair_counts", torch.zeros((0, 0), dtype=torch.long, device=device))
    ...
    def _suppress_overlapping_based_on_recent_occlusion(
        ...
        assert batch_size == num_ids, (
            f"Mask/metadata count mismatch in _suppress_overlapping: batch_size={batch_size}, num_ids={num_ids}, frame_idx={frame_idx}"
        )

        binary_tracker_low_res_masks_global = tracker_low_res_masks_global > 0
        if batch_size > 0:
            assert len(obj_ids_global) == batch_size, f"Mismatch in number of objects: {len(obj_ids_global)} vs {batch_size}"
            NEVER_OCCLUDED = -1
            ALWAYS_OCCLUDED = (
                100000  # This value should be larger than any possible frame index, indicates that the object was removed by hotstart logic
            )
            ...
            last_occluded_prev = torch.where(to_remove_mask, torch.full_like(last_occluded_prev, ALWAYS_OCCLUDED), last_occluded_prev)

            to_suppress = self._get_objects_to_suppress_based_on_most_recently_occluded(
                binary_tracker_low_res_masks_global, last_occluded_prev, obj_ids_global, frame_idx, reverse
            )
    ...
    def build_outputs(
        ...
        use_per_gpu_ids = len(existing_masklet_obj_ids_per_gpu) != len(existing_masklet_obj_ids_all) or not np.array_equal(
        ...
        existing_masklet_video_res_masks = F.interpolate(
            tracker_low_res_masks_global.unsqueeze(1), size=(orig_vid_height, orig_vid_width), mode="bilinear", align_corners=False
        ...
        if num_masks != num_ids:
            if num_masks < num_ids:
                pad = existing_masklet_video_res_masks.new_zeros((num_ids - num_masks, 1, orig_vid_height, orig_vid_width))
        ...
        new_masklet_video_res_masks = F.interpolate(
            new_det_low_res_masks, size=(orig_vid_height, orig_vid_width), mode="bilinear", align_corners=False
        )  # (num_obj, 1, H_video, W_video)
    ...
    def _get_objects_to_suppress_based_on_most_recently_occluded(
        ...
        to_suppress = torch.zeros(binary_low_res_masks.size(0), device=binary_low_res_masks.device, dtype=torch.bool)
        if len(obj_ids) <= 1:
            return to_suppress
            ...
            if filter_obj_ids is not None:
                if len(out_obj_ids) > 0:
                    ...
                    if len(filter_obj_ids) == 1 and max_mask_rows == 1 and max_score_rows == 1:
                        ...
                        keep_indices = [
                            i for i, oid in enumerate(out_obj_ids) if oid in filter_obj_ids and i < max_mask_rows and i < max_score_rows
                        ]
                else:
                    keep_indices = []
    ...
    def _associate_det_trk(
        ...
        if det_masks.shape[-2:] != trk_masks.shape[-2:]:
            ...
            if np.prod(det_masks.shape[-2:]) < np.prod(trk_masks.shape[-2:]):
                trk_masks = F.interpolate(trk_masks.unsqueeze(1), size=det_masks.shape[-2:], mode="bilinear", align_corners=False).squeeze(
                    1
                )
            else:
                # resize detections to track size
                det_masks = F.interpolate(det_masks.unsqueeze(1), size=trk_masks.shape[-2:], mode="bilinear", align_corners=False).squeeze(
                    1
                )
        ...
        with torch.profiler.record_function("associate_det_trk_compilable"):
            if trk_masks.shape[0] < self.max_num_objects:
                ...
                trk_masks_padded = torch.cat(
                    [trk_masks, torch.zeros(padding_size, *trk_masks.shape[1:], device=trk_masks.device, dtype=trk_masks.dtype)], dim=0
                )
            else:
        ...
        return LazyAssociateDetTrkResult(
            trk_is_unmatched, trk_is_nonempty, is_new_det, det_to_max_iou_trk_idx, det_is_high_conf, det_is_high_iou, det_keep, im_mask
        )
    ...
    def _process_hotstart_gpu(
        ...
        if prev_N_obj > 0:
            ...
            removed_mask_prev = gpu_metadata_prev["removed_mask"]  # (prev_N_obj,) - should be all False after compaction
        ...
        assert N_obj == prev_N_obj, f"N_obj ({N_obj}) should equal prev_N_obj ({prev_N_obj}); new objects handled after compaction"

        # Carry forward existing metadata (or initialize if first frame)
        NEVER_OCCLUDED = -1
        obj_first_frame = (
            obj_first_frame_prev if obj_first_frame_prev is not None else torch.full((N_obj,), frame_idx, dtype=torch.long, device=device)
        )
        ...
        trk_keep_alive = trk_keep_alive_prev if trk_keep_alive_prev is not None else torch.zeros(N_obj, dtype=torch.long, device=device)
        removed_mask = removed_mask_prev if removed_mask_prev is not None else torch.zeros(N_obj, dtype=torch.bool, device=device)
        overlap_pair_counts = (
        ...
        last_occluded = (
            last_occluded_prev if last_occluded_prev is not None else torch.full((N_obj,), NEVER_OCCLUDED, dtype=torch.long, device=device)
        )
        ...
        if self.decrease_trk_keep_alive_for_empty_masklets:
            trk_keep_alive = torch.where(
                ~adt_result.trk_is_nonempty, torch.clamp(trk_keep_alive - 1, min=self.min_trk_keep_alive), trk_keep_alive
            )
        ...
        remove_by_unmatch = is_within_hotstart & (consecutive_unmatch_count >= self.hotstart_unmatch_thresh) & ~removed_mask

        # Suppress if: keep_alive <= 0 AND not hotstart-only mode AND not removed
        suppress_by_unmatch = (
            (trk_keep_alive <= 0)
            & torch.tensor(not self.suppress_unmatched_only_within_hotstart).pin_memory().to(device=device, non_blocking=True)
            & ~removed_mask
            & ~remove_by_unmatch
        )
        ...
        if N_obj == 0:
            ...
            overlap_with_earlier = torch.where(is_earlier_matrix, overlap_pair_counts, torch.zeros_like(overlap_pair_counts))
            ...
            remove_by_overlap = is_within_hotstart & (max_overlap_with_earlier >= self.hotstart_dup_thresh) & ~removed_mask
    ...
    def _process_hotstart(
        ...
        for obj_id, frame_indices in unmatched_frame_inds.items():
            ...
            if len(frame_indices) >= self.hotstart_unmatch_thresh:
                ...
                if is_within_hotstart:
                    ...
                    logger.info(f"Removing object {obj_id} at frame {frame_idx} since it is unmatched for frames: {frame_indices}")
            if (
        ...
        for (first_obj_id, obj_id), frame_indices in overlap_pair_to_frame_inds.items():
            if obj_id in removed_obj_ids or obj_id in obj_ids_newly_removed:
                continue  # skip if the object is already removed
            if (obj_first_frame_idx[obj_id] > hotstart_diff and not reverse) or (obj_first_frame_idx[obj_id] < hotstart_diff and reverse):
    ...
    def _tracker_update_memories(
        self, sam2_inference_states: List[Any], frame_idx: int, tracker_metadata: Dict[str, Any], low_res_masks: Tensor
    ):
        """
        ...
        high_res_masks = F.interpolate(low_res_masks.unsqueeze(1), size=(high_res_H, high_res_W), mode="bilinear", align_corners=False)
        ...
        for state_i, sam2_state in enumerate(sam2_inference_states):
            ...
            if self.is_multiplex and self.tracker.is_multiplex_dynamic:
                local_idx = torch.tensor(object_idx_assignment[state_i]).pin_memory().to(device=high_res_masks.device, non_blocking=True)
            ...
            encoded_mem = self.tracker._run_memory_encoder(
                sam2_state, frame_idx, local_batch_size, local_high_res_masks, local_object_score_logits, is_mask_from_pts=False
            )
            if self.is_multiplex:
                (local_maskmem_features, local_maskmem_pos_enc, local_image_features, local_image_pos_enc) = encoded_mem
            else:
                ...
                if self.is_multiplex:
                    ...
                    if self.reapply_no_object_pointer:
                        ...
                        newly_suppressed_objects = (
                            output_dict[storage_key][frame_idx]["object_score_logits"] > self.tracker.object_score_logit_threshold
    ...
    def _tracker_add_new_objects(
        ...
        if self.tracker.is_multiplex_dynamic:
            ...
            if best_state is not None:
                ...
                new_sam2_state = self.tracker.init_state(
                    cached_features=feature_cache, video_height=orig_vid_height, video_width=orig_vid_width, num_frames=num_frames
                )
                new_sam2_state["backbone_out"] = prev_sam2_state.get("backbone_out", None) if prev_sam2_state is not None else None
        ...
        if self.is_multiplex:
            ...
            self.tracker.add_new_masks(
                inference_state=new_sam2_state, frame_idx=frame_idx, obj_ids=new_obj_ids, masks=new_obj_masks, add_mask_to_memory=True
            )
        else:
            # add object one by one
            for new_obj_id, new_mask in zip(new_obj_ids, new_obj_masks):
                self.tracker.add_new_mask(
                    inference_state=new_sam2_state, frame_idx=frame_idx, obj_id=new_obj_id, mask=new_mask, add_mask_to_memory=True
                )
    ...
    def _tracker_remove_objects(self, tracker_states_local: List[Any], obj_ids: list[int]):
        ...
        if self.is_multiplex:
            ...
            for sam2_inference_state in tracker_states_local_before_removal:
                ...
                new_obj_ids, _ = self.tracker.remove_objects(sam2_inference_state, obj_ids, strict=False, need_output=False)
...
class Sam3MultiplexPredictorWrapper(Sam3MultiplexTrackerPredictor):
    ...
    def __init__(self, model, per_obj_inference=False, fill_hole_area=0, is_multiplex=True, is_multiplex_dynamic=True):
