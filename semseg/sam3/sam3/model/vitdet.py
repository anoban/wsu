...
class Mlp(nn.Module):
    ...
    def __init__(
        self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, norm_layer=None, bias=True, drop=0.0, use_conv=False
    ):
        super().__init__()
...
def get_rel_pos(q_size: int, k_size: int, rel_pos: Tensor) -> Tensor:
    ...
    if rel_pos.shape[0] != max_rel_dist:
        ...
        rel_pos_resized = F.interpolate(
            rel_pos.reshape(1, rel_pos.shape[0], -1).permute(0, 2, 1), size=max_rel_dist, mode="linear", align_corners=False
        )
...
class Attention(nn.Module):
    ...
    def _setup_rope_freqs(self) -> None:
        ...
        if self.use_ve_rope:
            ...
            self.rope = VisionRotaryEmbeddingVE(dim=self.head_dim // 2, seq_len=self.input_size[0], pt_seq_len=self.rope_pt_size[0])
            return

        # initialize 2d rope freqs
        self.compute_cis = partial(compute_axial_cis, dim=self.head_dim, theta=self.rope_theta)
        ...
        if self.rope_pt_size != self.input_size and self.rope_tiled:
            ...
            rh, rw = (self.input_size[0] // self.rope_pt_size[0], self.input_size[1] // self.rope_pt_size[1])
            assert rh >= 1, rw >= 1
            ...
            freqs_cis = freqs_cis.reshape(self.rope_pt_size[0], self.rope_pt_size[1], -1).tile(rh, rw, 1).reshape(-1, freqs_cis.shape[-1])
        else:
            # interpolate rope
            scale_pos = 1.0
            if self.rope_interp:
            ...
            freqs_cis = self.compute_cis(end_x=self.input_size[0], end_y=self.input_size[1], scale_pos=scale_pos)
        if self.cls_token:
    ...
    def _apply_rope(self, q, k) -> Tuple[Tensor, Tensor]:
        ...
        if self.use_rope_real:
            return apply_rotary_enc_real(q, k, freqs_cis_imag=self.freqs_cis_imag, freqs_cis_real=self.freqs_cis_real)
