...
def _nms_masks_core(
    pred_probs: torch.Tensor, pred_masks: torch.Tensor, prob_threshold: float, iou_threshold: float, nms_use_iom: bool = False
) -> torch.Tensor:
...
def _batched_generic_nms_mask(ious: torch.Tensor, scores: torch.Tensor, is_valid: torch.Tensor, iou_threshold: float) -> torch.Tensor:
...
def generic_nms_mask(ious: torch.Tensor, scores: torch.Tensor, is_valid: torch.Tensor, iou_threshold=0.5) -> torch.Tensor:
    """
    A generic version of `torchvision.ops.nms` that takes a pairwise IoU matrix. (CPU implementation
