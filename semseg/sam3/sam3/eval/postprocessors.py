...
class PostProcessImage(nn.Module):
    ...
    def forward(
        ...
        boxes, scores, labels, keep = self._process_boxes_and_labels(target_sizes_boxes, forced_labels, out_bbox, out_probs)
    ...
    def _process_masks(self, target_sizes, pred_masks, consistent=True, keep=None):
        ...
        if consistent:
            ...
            out_masks = interpolate(pred_masks, target_size.squeeze().tolist(), mode="bilinear", align_corners=False).sigmoid() > 0.5
            if self.convert_mask_to_rle:
            ...
            for i, mask in enumerate(pred_masks):
                ...
                try:
                    interpolated = interpolate(mask.unsqueeze(1), (h, w), mode="bilinear", align_corners=False).sigmoid() > 0.5
                except Exception:
    ...
    def process_results(self, find_stages, find_metadatas: List[BatchedInferenceMetadata], **kwargs):
        ...
        for outputs, meta in zip(find_stages, find_metadatas):
            img_size_for_boxes = meta.original_size if self.use_original_sizes_box else torch.ones_like(meta.original_size)
            img_size_for_masks = meta.original_size if self.use_original_sizes_mask else torch.ones_like(meta.original_size)
...
class PostProcessAPIVideo(PostProcessImage):
    ...
    def process_results(self, find_stages, find_metadatas: List[BatchedInferenceMetadata], **kwargs):
        ...
        for frame_idx, (frame_outs, meta) in enumerate(zip(find_stages, find_metadatas)):
            ...
            frame_outs_td = TensorDict({k: frame_outs[k] for k in self.EXPECTED_KEYS}).auto_batch_size_()  # Shape is [P,Q,...]
            ...
            tracked_obj_ids = [
                ...
                for p_id, q_id in zip(tracked_obj_ids_idx[PROMPT_AXIS], tracked_obj_ids_idx[OBJ_QUERY_AXIS])
            ]
        ...
        padded_frames_results = TensorDict(
            {k: torch.zeros(num_preds, num_frames, *v.shape[1:], device=v.device, dtype=v.dtype) for k, v in vid_preds_packed.items()},
            batch_size=[num_preds, num_frames],
        )
...
class PostProcessTracking(PostProcessImage):
    ...
    def __init__(self, max_dets_per_img: int, iou_type="bbox", force_single_mask: bool = False, **kwargs) -> None:
            ...
            results.update(
                {
                    ...
                    for media_id, object_id, frame_index, result in zip(
                        meta.original_image_id, meta.object_id, meta.frame_index, detection_results
                    )
                }
            )
        return results
...
class PostProcessCounting(nn.Module):
    ...
    def __init__(self, use_original_ids: bool = False, threshold: float = 0.5, use_presence: bool = False) -> None:
        """
        Args:
