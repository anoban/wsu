...
from sam3.train.utils.distributed import all_gather, gather_to_rank_0_via_filesys, get_rank, is_main_process
        ...
        if imgIds is not None:
            ...
            for coco_eval in self.coco_evals[0].values():
                ...
                coco_eval.evalImgs = list(
                    np.asarray(coco_eval.evalImgs).reshape(len(catIds), len(p.areaRng), len(old_img_ids))[..., id_mask].flatten()
                )
                accumulate(coco_eval, use_self_eval=self.use_self_evaluate)
        ...
        for i, coco_gt in enumerate(self.coco_gts):
            for iou_type in self.iou_types:
                ...
                if self.use_normalized_areas:
                    ...
                    self.coco_evals[i][iou_type].params.areaRngLbl = ["all", "tiny", "small", "medium", "large", "huge", "whole_image"]
    ...
    def prepare_for_coco_segmentation(self, predictions):
        ...
        for original_id, prediction in predictions.items():
            ...
            for k, rle in enumerate(rles):
                payload = {"image_id": original_id, "category_id": labels[k], "segmentation": rle, "score": scores[k], "area": areas[k]}
...
def create_common_coco_eval(coco_eval, img_ids, eval_imgs, use_self_evaluate, gather_pred_via_filesys=False, metrics_dump_dir=None):
