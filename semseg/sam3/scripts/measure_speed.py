...
def synthesize_video_data(num_objects: int, out_dir: str, radius: int, speed: int, width: int, height: int, n_frames: int):
    ...
    for i in range(n_frames):
        ...
        for obj_idx in range(num_objects):
            ...
            draw.ellipse([(rx - radius, ry - radius), (rx + radius, ry + radius)], fill=circle_colors[obj_idx])
            vx, vy = velocities[obj_idx]
            x += vx
            y += vy
            positions[obj_idx] = [np.clip(x, radius, width - radius), np.clip(y, radius, height - radius)]
...
def main_loop(model_wrapper, session_id, text_prompt):
    ...
    model_wrapper.handle_request({"type": "add_prompt", "session_id": session_id, "frame_index": 0, "text": text_prompt})
...
def run_test(
    ...
    if synthesize_data:
        synthesize_video_data(
            num_objects=num_objects, out_dir=video_dir, radius=radius, speed=speed, width=width, height=height, n_frames=n_frames
        )

    from sam3 import build_sam3_predictor

    print(f"Building {version} model...")
    build_kwargs = dict(version=version, compile=do_compile, async_loading_frames=False)
    if checkpoint_path:
    ...
    for _ in range(NUM_WARMUP_TRIES):
        fps = max(main_loop(model_wrapper=model_wrapper, session_id=session_id, text_prompt="circle"), fps)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--version", type=str, default="sam3.1", choices=["sam3", "sam3.1"], help="Model version (default: sam3.1)")
    parser.add_argument("--checkpoint", type=str, default=None, help="Path to checkpoint (auto-downloads from HuggingFace if not provided)")
