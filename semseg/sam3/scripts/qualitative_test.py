...
def synthesize_video(out_dir, num_objects=5, n_frames=30, width=1024, height=1024):
    ...
    positions = [[float(np.random.randint(80, width - 80)), float(np.random.randint(80, height - 80))] for _ in range(num_objects)]
    ...
    for i in range(n_frames):
        ...
        for j in range(num_objects):
            ...
            vx, vy = velocities[j]
            positions[j] = [np.clip(x + vx, 50, width - 50), np.clip(y + vy, 50, height - 50)]
...
def load_frame(frame_dir, frame_idx):
    return cv2.cvtColor(cv2.imread(os.path.join(frame_dir, f"{frame_idx:05d}.jpg")), cv2.COLOR_BGR2RGB)
...
def main():
    ...
    parser.add_argument("--video", type=str, default=None, help="Path to video file. If not provided, generates synthetic video.")
    ...
    model.handle_request({"type": "add_prompt", "session_id": session_id, "frame_index": 0, "text": args.text_prompt})
