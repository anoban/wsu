...
class SAM2VideoPredictor(SAM2Base):
    ...
    def add_new_points_or_box(
        self, inference_state, frame_idx, obj_id, points=None, labels=None, clear_old_points=True, normalize_coords=True, box=None
    ):
        ...
        consolidated_out = self._consolidate_temp_output_across_obj(
            inference_state, frame_idx, is_cond=is_cond, consolidate_at_video_res=True
        )
        ...
        for obj_idx in range(batch_size):
            ...
            if obj_mask.shape[-2:] == consolidated_pred_masks.shape[-2:]:
                ...
                resized_obj_mask = torch.nn.functional.interpolate(
                    obj_mask, size=consolidated_pred_masks.shape[-2:], mode="bilinear", align_corners=False
                )
...
class SAM2VideoPredictorVOS(SAM2VideoPredictor):
    ...
    def _compile_all_components(self):
        ...
        self.memory_encoder.forward = torch.compile(self.memory_encoder.forward, mode="max-autotune", fullgraph=True, dynamic=False)
