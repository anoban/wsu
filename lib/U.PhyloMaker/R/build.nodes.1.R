build.nodes.1 <- function(tree, tips) {
    ...
    clustF.size <- data.frame(
        numF = as.integer(names(clustF.size)),
        sizeF = as.integer(clustF.size)
    )
    clustG.size <- table(clustG)
    clustG.size <- data.frame(
        numG = as.integer(names(clustG.size)),
