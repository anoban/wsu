phylo.maker <- function(sp.list, tree, gen.list, nodes.type = 1, output.sp.list = TRUE,
    ...
    rnN <- data.frame(node.label = paste("N", 1:tree$Nnode,
        sep = ""
    ), oriN = tree$node.label, stringsAsFactors = FALSE)
    ...
    if (2 %in% scenarios) {
        ...
        for (o in 1:r) {
            ...
            if (dim(data)[1] > 0) {
                for (i in 1:dim(data)[1]) {
                    ...
                    if (g == 1 & s == 1) {
                        ...
                        t2 <- ext.node(t2,
                            location.tip = num, tip.label = data$species[i],
            ...
            if (dim(data)[1] > 0) {
                for (i in 1:dim(data)[1]) {
                    ...
                    if (n == 1) {
                        ...
                        x <- which(t2$node.label == nlabel)
                        xx <- rbind(rnN2[1:(x - 1), ], data.frame(
                            node.label = nlabel,
            ...
            tree2 <- t2
            tree2$node.label <- rnN2$oriN[match(
                tree2$node.label,
                rnN2$node.label
            )]
    ...
    if (3 %in% scenarios) {
        ...
        if (dim(data)[1] > 0) {
            for (i in 1:dim(data)[1]) {
                ...
                if (g == 1 & s == 1) {
                    ...
                    xx <- rbind(rnN3[1:(x - 1), ], data.frame(
                        node.label = nlabel,
                        oriN = ""
                    ))
        ...
        if (dim(data)[1] > 0) {
            for (i in 1:dim(data)[1]) {
                ...
                if (length(n) == 1) {
                    t3 <- ext.node(t3,
                        location.tip = n, tip.label = data$species[i],
                        node.label = nlabel, position = 1 / 2
                    )
