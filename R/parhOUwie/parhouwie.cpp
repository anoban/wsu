...
namespace paths {
    ...
    static constexpr wchar_t PHYLOGENY[] { L"./../../data/chapter2/uphylomaker/FRED4_1292.tre" };
    static constexpr wchar_t TRAIT_DATA[] { L"./../../data/chapter2/FRED/subsets/name_matched_FRED4_1292.csv" };
    static constexpr wchar_t SAVE_RDS[] { L"./../../data/chapter2/rdata/parallel/rd_1292_100sims/" }; // must end with a foward slash
}
...
static constexpr unsigned long long MAX_PARALLEL_PROCS { 0xC }; // with 12 the CPU gets very close maxxing out
...
namespace utils {
    ...
    [[nodiscard, clang::always_inline]] static inline const wchar_t* __stdcall error_code_to_wstring(_In_ const unsigned long& _errcode) noexcept {
        ...
        unsigned long nbyteswritten =
            ::FormatMessageW(FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_MAX_WIDTH_MASK, nullptr, _errcode, 0, buffer, ERRORMSG_BUFFSIZE, nullptr);
        ...
        if (!nbyteswritten) { // will be 0 if the call above to FormatMessageW failed; if that, the error string is not found in the system, try Ntdsbmsg.dll
            ...
            nbyteswritten = ::FormatMessageW(
                FORMAT_MESSAGE_FROM_HMODULE | FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_MAX_WIDTH_MASK, handle_ntdsbmsg, _errcode, 0, buffer, ERRORMSG_BUFFSIZE, nullptr
            );
    ...
    [[nodiscard]] static inline bool __stdcall handle_parallel_waits(
        _Inout_ std::vector<HANDLE64>& _phandles, _Inout_ std::vector<HANDLE64>& _thandles, _In_ const bool& _all, _In_ const unsigned long& _duration
    ) noexcept {
        ...
        const unsigned long waitstatus =        ::WaitForMultipleObjects( // make sure that the return value is valid
                        _phandles.size(),
                        _phandles.data(),
                        _all, // return when at least one process is signalled
                        _duration // INFINITE milliseconds is about 1193 hours, so we can just use that
                    );
        ...
        switch (waitstatus) {
            case WAIT_FAILED :
                ...
                if (_all) goto CLOSE_ACTIVE_HANDLES_AND_EXIT; // if the wait was for all processes, close all the handles and return false
                return false;                                 // else just return false

            case WAIT_TIMEOUT :
                ::fputws(L"WaitForMultipleObjects signalled WAIT_TIMEOUT\n", stderr);
                if (_all) goto CLOSE_ACTIVE_HANDLES_AND_EXIT;
                return false;
        ...
        if ( // (waitstatus >= WAIT_OBJECT_0) && // will always be true because the return value of ::WaitForMultipleObjects() is unsigned
            waitstatus < (WAIT_OBJECT_0 + _phandles.size())
        ) {
            exitval = true;
            if (_all) goto CLOSE_ACTIVE_HANDLES_AND_EXIT;
            // bWaitAll == FALSE
        ...
        else if ((waitstatus >= WAIT_ABANDONED_0) && (waitstatus < (WAIT_ABANDONED_0 + _phandles.size()))) {
            if (_all) {
...
CLOSE_ACTIVE_HANDLES_AND_EXIT:                                 // close all the leftover process handles and thread handles
        for (unsigned long i = 0; i < _phandles.size(); ++i) { // taking it for granted that phandles.size()==thandles.size()
            ...
            ::CloseHandle(_phandles.at(i));
            ::CloseHandle(_thandles.at(i));
        }
...
CLOSE_SELECTED_HANDLE_AND_EXIT:
        ...
        // else
        //     excodes.push_back(exitcode);
        ::CloseHandle(*(_phandles.data() + handle_offset));
        _phandles.erase(_phandles.begin() + handle_offset); // remove the signalled process
        ::CloseHandle(*(_thandles.data() + handle_offset));
        _thandles.erase(_thandles.begin() + handle_offset); // remove the signalled thread handle
        return exitval;
    }
...
namespace houwie {
    ...
    [[clang::always_inline, nodiscard]] static inline constexpr const wchar_t* __stdcall dmod_tostr(_In_ const DISCRETE_MODEL& _model) noexcept {
        switch (_model) {
    ...
    [[clang::always_inline, nodiscard]] static inline constexpr const wchar_t* __stdcall cmod_tostr(_In_ const CONTINUOUS_MODEL& _model) noexcept {
        switch (_model) {
    ...
    [[clang::always_inline, nodiscard]] static inline const wchar_t* __stdcall rds_path(
        _In_ const DISCRETE_MODEL&   _dmod,
        _In_ const CONTINUOUS_MODEL& _cmod,
        _In_ const wchar_t* const    _trait,   // e.g. F00727
        _In_ const wchar_t* const    _savedir, // assumed ends with a forward slash, e.g. "C:/Users/Documents/"
        _In_ const bool&             _nulL,
        _In_ const wchar_t* const    _suffix // e.g. _1006sp
    ) noexcept {
        ...
        if (::wcsrchr(_savedir, L'/') != (_savedir + ::wcslen(_savedir) - 1)) { // exit if savedir does not end with a foward slash
        ...
        if (_suffix) // e.g. C:/Users/Documents/ARDOUMV_F00679_CD_395sp.Rds
            ::swprintf_s(
                buffer,
                SAVERDS_NAME_LENGTH,
            ...
            ::swprintf_s(buffer, SAVERDS_NAME_LENGTH, L"%s%s%s_%s_%s.Rds", _savedir, dmod_tostr(_dmod), cmod_tostr(_cmod), _trait, _nulL ? L"CID" : L"CD");

        return buffer;
    }

    [[clang::always_inline]] static inline bool __stdcall generate_rscript(
        _Inout_ std::wstring&          _buffer,
        _In_ const wchar_t* const      _phylogeny,
        _In_ const wchar_t* const      _traitdata,
        _In_ const wchar_t* const      _trait,
        _In_ const DISCRETE_MODEL&     _dmod,
        _In_ const CONTINUOUS_MODEL&   _cmod,
        _In_ const wchar_t* const      _savedir,
        _In_ const wchar_t* const      _suffix,
        _In_ const bool&               _null,
        _In_ const unsigned long long& _nsims
    ) noexcept {
        // make sure that all the paths are valid, using separate conditional for detailed error reporting
        if (!::PathFileExistsW(_savedir)) {
            ::fwprintf_s(stderr, L"Invalid argument savedir in call to " __FUNCTIONW__ ". %s\n", utils::error_code_to_wstring(::GetLastError()));
            return false;
        }

        if (!::PathFileExistsW(_phylogeny)) {
            ::fwprintf_s(stderr, L"Invalid argument phylogeny in call to " __FUNCTIONW__ ". %s\n", utils::error_code_to_wstring(::GetLastError()));
            return false;
        }

        if (!::PathFileExistsW(_traitdata)) {
            ::fwprintf_s(stderr, L"Invalid argument traitdata in call to " __FUNCTIONW__ ". %s\n", utils::error_code_to_wstring(::GetLastError()));
            return false;
        }

        if (_buffer.size() < RSCRIPT_BUFFSIZE) _buffer.resize(RSCRIPT_BUFFSIZE);
        ::memset(_buffer.data(), 0, _buffer.size() * sizeof(wchar_t)); // clean up the buffer before every new write
        const wchar_t* const rds_savepath = rds_path(_dmod, _cmod, _trait, _savedir, _null, _suffix);
...
struct launch_result {
        int       _exitcode;
        int       __; // padding
        long long _nsucceeded;
};

[[nodiscard]] static inline launch_result __stdcall launch_fits_in_parallel(
    _In_ const wchar_t* const      _rexe,
    _In_ const wchar_t* const      _phylogeny,
    _In_ const wchar_t* const      _data,
    _In_ const wchar_t* const      _savepath,
    _In_ const wchar_t* const      _trait,
    _In_ const unsigned long long& _nsims,
    _In_ const unsigned long long& _max_parallel_procs
) noexcept {
    ...
    launch_result         result { ._exitcode = EXIT_SUCCESS, ._nsucceeded = 0 };
    ...
    for (unsigned dmod = 0; dmod < 3; ++dmod) {     // discrete models
        for (unsigned cmod = 0; cmod < 4; ++cmod) { // continuous models
            for (unsigned nm = 0; nm < 2; ++nm) {   // null model (0, 1) i.e true or false
                ...
                ::swprintf_s(cmdline.data(), cmdline.size(), L"%s --no-save -e \"%s\"", _rexe, rscript.c_str());
                ...
                if (active_process_handles.size() >= _max_parallel_procs) {
                ...
                if (!::CreateProcessW(
                        _rexe, // DO NOT LEAVE THIS EMPTY!!! i.e. nullptr
                        cmdline.data(),
                        nullptr,
                        nullptr,
                        TRUE,
    ...
    if (!utils::handle_parallel_waits(active_process_handles, active_thread_handles, true, INFINITE)) result._exitcode = EXIT_FAILURE;
...
int wmain() {
    ...
    const auto results { ::launch_fits_in_parallel(paths::RINTERPRETER, paths::PHYLOGENY, paths::TRAIT_DATA, paths::SAVE_RDS, fred::ROOT_DIAMETER, 300, MAX_PARALLEL_PROCS) };

    ::QueryPerformanceCounter(&stop);

    ::wprintf_s(
        L"Done, %llu launches completed within %.3Lf hours!\n",
        results._nsucceeded,
        (stop.QuadPart - start.QuadPart) / (freq.QuadPart * 3600.00L) // NOLINT(cppcoreguidelines-narrowing-conversions)
    );

    return results._exitcode;
}
