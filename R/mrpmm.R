...
model_0 <- brms::brm(brms::brmsformula(mvbind(F00727, F00679, F00709) ~ state + (1|p|gr(binominal, cov = corrmat)) + (1|q|taxa)) + set_rescor(TRUE),
                     data = fred4, data2 = list(corrmat = corrmat), chains = 18, cores = 18, threads = 4, iter = 10000)
saveRDS(object = model_0, file = "./ScratchData/brms_model_0.Rds")


model_1 <- brms::brm(brms::brmsformula(mvbind(F00727, F00679, F00709) ~ state + (1|p|taxa)) + set_rescor(TRUE),
                     data = fred4, data2 = list(corrmat = corrmat), chains = 18, cores = 18, threads = 4, iter = 10000)
saveRDS(object = model_1, file = "./ScratchData/brms_model_1.Rds")
